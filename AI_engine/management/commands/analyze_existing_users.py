...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if user_id:
            ...
            users_with_data = User.objects.annotate(
                ...
                journal_count=Count('journal_entries')  # Changed from 'journalentry' to 'journal_entries'
            ).filter(
                mood_count__gt=0,
                journal_count__gt=0
            )
            ...
            if kwargs.get('concurrent'):
                ...
                with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
                    ...
                    for user in users_with_data:
                        futures.append(
                            executor.submit(self.analyze_user, user, days, kwargs.get('dry_run', False))
                        )
                    ...
                    for future in concurrent.futures.as_completed(futures):
                        try:
                            ...
                            self.stderr.write(self.style.ERROR(f"Error in worker thread: {str(e)}"))
            else:
    ...
    def analyze_user(self, user, days, dry_run=False):
        ...
        try:
            ...
            if dry_run:
                self.stdout.write(f"DRY RUN: Would analyze user {user.username} (ID: {user.id})")
                # Preview analysis without saving
                preview = ai_service.analyze_user_data(user, date_range=days, dry_run=True)
                self.stdout.write(
            ...
            if result:
                ...
                self.stdout.write(
                    self.style.WARNING(
                        f"⚠ Analysis failed for {user.username}"
                    )
                )
            ...
            self.stderr.write(
                self.style.ERROR(
                    f"Error analyzing user {user.username}: {str(e)}"
                )
            )
