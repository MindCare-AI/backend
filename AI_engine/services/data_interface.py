...
class AIDataInterfaceService:
    ...
    def _initialize_datawarehouse(self):
        ...
        try:
            from datawarehouse.services.unified_data_collection_service import unified_data_collector
            self.datawarehouse_service = unified_data_collector
            logger.info("AI Data Interface Service initialized with datawarehouse connection")
        except ImportError as e:
            logger.error(f"Failed to initialize datawarehouse connection: {e}")
            self.datawarehouse_service = None
    
    def get_ai_ready_dataset(self, user_id: int, period_days: int = 30, 
    ...
    def get_journal_analytics(self, user_id: int, period_days: int = 30) -> Dict[str, Any]:
        """
    ...
    def get_behavioral_patterns(self, user_id: int, period_days: int = 30) -> Dict[str, Any]:
        """
    ...
    def get_communication_metrics(self, user_id: int, period_days: int = 30) -> Dict[str, Any]:
        """
    ...
    def get_therapy_session_data(self, user_id: int, period_days: int = 30) -> Dict[str, Any]:
        """
    ...
    def get_social_engagement_data(self, user_id: int, period_days: int = 30) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.error(f"Error getting social engagement data for user {user_id}: {e}")
            return {'status': 'error', 'error': str(e)}
    
    def get_cross_domain_insights(self, user_id: int, period_days: int = 30) -> Dict[str, Any]:
        """
    ...
    def get_data_quality_metrics(self, user_id: int, period_days: int = 30) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            return {
                ...
                'analysis_recommendation': quality_metrics.get('analysis_recommendation', 'insufficient')
            }
        except Exception as e:
    ...
    def check_analysis_readiness(self, user_id: int, analysis_type: str, 
        ...
        try:
            ...
            result = {
                ...
                'completeness': quality_metrics.get('completeness', 0.0),
                'recommendation': quality_metrics.get('analysis_recommendation', 'insufficient')
            }
            
            # Add specific guidance
            if not is_ready:
                result['guidance'] = self._get_readiness_guidance(analysis_type, quality_metrics)
        ...
        try:
            ...
            summary = {
                ...
                'mood_summary': {
                    ...
                    'trend': mood_data.get('trend_analysis', {}).get('trend', 'unknown')
                },
                'journal_summary': {
                    'entries_count': journal_data.get('total_entries', 0),
                    'total_words': journal_data.get('total_words', 0),
                    'writing_consistency': journal_data.get('writing_consistency', {}).get('score', 0)
                },
                'available_domains': []
            }
    ...
    def _get_readiness_guidance(self, analysis_type: str, quality_metrics: Dict[str, Any]) -> Dict[str, Any]:
        """
