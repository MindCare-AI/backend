...
from django.db.models import Avg, Case, When, FloatField, Count, Q
...
class PredictiveAnalysisService:
    ...
    def predict_mood_decline(self, user, timeframe_days: int = 7) -> Dict:
        ...
        try:
            ...
            if not mood_logs.exists():
                result = {"risk_level": "unknown", "confidence": 0, "factors": [], "message": "Insufficient data"}
            ...
            statistical_risk = self._calculate_statistical_risk(mood_logs, timeframe_days)
    ...
    def _prepare_mood_data_with_trends(self, mood_logs, timeframe_days: int) -> Dict:
        ...
        recent_moods = mood_values[-timeframe_days:] if len(mood_values) >= timeframe_days else mood_values
    ...
    def _calculate_mood_trends(self, mood_values: List[float], timeframe_days: int) -> Dict:
        """Calculate various trend indicators"""
        if len(mood_values) < 3:
            return {"trend": "insufficient_data", "slope": 0, "acceleration": 0}
            
        recent_values = mood_values[-timeframe_days:] if len(mood_values) >= timeframe_days else mood_values
        ...
        return {
            ...
            "recent_change": float(recent_values[-1] - recent_values[0]) if len(recent_values) > 1 else 0
        }
    ...
    def _detect_mood_patterns(self, mood_logs) -> Dict:
        ...
        patterns["activity_correlation"] = {
            activity: np.mean(moods) for activity, moods in activity_moods.items()
        ...
        if len(patterns["activity_correlation"]) > 0:
            lowest_activity = min(patterns["activity_correlation"].items(), key=lambda x: x[1])
            if lowest_activity[1] < 3:
                patterns["concerning_patterns"].append(f"negative_activity_correlation_{lowest_activity[0]}")
    ...
    def _get_contextual_data(self, user, timeframe_days: int) -> Dict:
        ...
        try:
            ...
            if recent_entries.exists():
                ...
                content_analysis = self._analyze_journal_content_for_context(recent_entries)
    ...
    def _analyze_journal_content_for_context(self, entries) -> Dict:
        ...
        patterns = {
            "sleep_mentions": [r'\bsleep\b', r'\btired\b', r'\bexhausted\b', r'\binsomnia\b'],
            "stress_mentions": [r'\bstress\b', r'\banxious\b', r'\bworried\b', r'\boverwhelmed\b'],
            "medication_mentions": [r'\bmedication\b', r'\bpill\b', r'\bdose\b', r'\bside effect\b'],
            "social_mentions": [r'\bfriend\b', r'\bfamily\b', r'\balone\b', r'\bisolated\b'],
            "work_mentions": [r'\bwork\b', r'\bjob\b', r'\bcareer\b', r'\bboss\b'],
            "relationship_mentions": [r'\brelationship\b', r'\bpartner\b', r'\bdating\b', r'\bmarriage\b']
        }
...
"""
    ...
    def _calculate_statistical_risk(self, mood_logs, timeframe_days: int) -> Dict:
        ...
        risk_indicators = {
            ...
            "below_baseline_percentage": self._calculate_below_baseline_percentage(mood_values, recent_values),
            "trend_deterioration": self._assess_trend_deterioration(mood_values, timeframe_days)
        }
    ...
    def _count_consecutive_low_days(self, values: List[float], threshold: float = 3.0) -> int:
    ...
    def _count_significant_drops(self, values: List[float], drop_threshold: float = 2.0) -> int:
    ...
    def _calculate_below_baseline_percentage(self, all_values: List[float], recent_values: List[float]) -> float:
    ...
    def _assess_trend_deterioration(self, values: List[float], timeframe_days: int) -> bool:
    ...
    def predict_therapy_outcomes(self, user, timeframe_days: int = 30) -> Dict:
        ...
        try:
            ...
            logger.error(f"Error in therapy outcome prediction: {str(e)}", exc_info=True)
            return self._create_default_prediction("therapy_prediction_error")

    def _calculate_enhanced_engagement_metrics(self, user, start_date, end_date, appointments_available: bool) -> Dict:
    ...
    def _calculate_content_quality_score(self, user, start_date, end_date) -> float:
        ...
        try:
            ...
            for entry in entries:
                ...
                reflection_words = ['feel', 'think', 'realize', 'understand', 'learned', 'noticed']
                score += min(0.3, sum(0.05 for word in reflection_words if word in content))
                
                # Emotional awareness
                emotion_words = ['angry', 'sad', 'happy', 'anxious', 'grateful', 'frustrated']
                score += min(0.2, sum(0.02 for word in emotion_words if word in content))
    ...
    def _calculate_progress_indicators(self, user, timeframe_days: int) -> Dict:
        ...
        try:
            ...
            improvement_score = (recent_avg - earlier_avg) / 10  # Normalized improvement
...
"""
    ...
    def analyze_journal_patterns(self, user, time_field="created_at", **kwargs) -> Dict:
        ...
        try:
            ...
            return {"concerns": False, "sentiment_trend": "neutral", "avg_mood": 3.0, "error": str(e)}

    def _analyze_journal_categories(self, queryset) -> Dict:
        """Analyze journal entries by category with enhanced metrics"""
        try:
            categories = JournalCategory.objects.filter(user=queryset.first().user if queryset.exists() else None)
            category_analysis = {}
            ...
            for category in categories:
                ...
                if category_entries.exists():
                    ...
                    category_analysis[category.name] = {
                        ...
                        "most_recent": category_entries.latest('created_at').content[:100],
                        "frequency_score": self._calculate_frequency_score(category_entries),
                        "engagement_score": self._calculate_category_engagement(category_entries)
                    }
    ...
    def _calculate_frequency_score(self, queryset) -> float:
        ...
        days_span = (queryset.latest('created_at').created_at - queryset.earliest('created_at').created_at).days + 1
    ...
    def _detect_emotion_patterns(self, content: str) -> Dict:
        ...
        emotion_keywords = {
            "anxiety": [r'\banxious\b', r'\bworried\b', r'\bnervous\b', r'\bstressed\b'],
    ...
    def _analyze_journal_temporal_patterns(self, queryset) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "most_active_hour": max(hour_dist.items(), key=lambda x: x[1])[0] if hour_dist else 12,
                "most_active_day": max(day_dist.items(), key=lambda x: x[1])[0] if day_dist else "Unknown"
            }
    ...
    def _identify_journal_concerns(self, queryset) -> bool:
        ...
        try:
            ...
            concerning_keywords = [
                r'\bsuicid\w*\b', r'\bself.harm\b', r'\bhurt.myself\b',
                r'\bcan.?t.cope\b', r'\bwant.to.die\b', r'\bhopeless\b',
                r'\bcrisis\b', r'\bemergency\b'
            ]
            ...
            if negative_entries.count() > queryset.count() * 0.7:  # More than 70% negative
                return True
    ...
    def _create_default_prediction(self, error_type: str) -> Dict:
        ...
        return defaults.get(error_type, {
            "status": "error",
            "confidence": 0,
    ...
    def _create_overall_assessment(self, mood_pred: Dict, therapy_pred: Dict, journal_analysis: Dict) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "priority_level": "urgent" if overall_risk == "high" else "medium" if overall_risk == "medium" else "routine"
            }
