...
class SocialInteractionAnalysisService:
    ...
    def __init__(self):
        ...
        self.cache_timeout = getattr(settings, 'AI_ENGINE_SETTINGS', {}).get('CACHE_TIMEOUT', 900)
        self.max_prompt_length = getattr(settings, 'AI_ENGINE_SETTINGS', {}).get('MAX_PROMPT_LENGTH', 4000)
    ...
    def analyze_social_interactions(self, user, days: int = None) -> Dict[str, Any]:
        ...
        try:
            ...
            interaction_data = self._collect_interaction_data(user, start_date, end_date)
            ...
            engagement_metrics = self._calculate_advanced_engagement_metrics(interaction_data)
            ...
            logger.error(f"Error analyzing social interactions: {str(e)}", exc_info=True)
    ...
    def _collect_interaction_data(self, user, start_date, end_date) -> Dict[str, Any]:
        ...
        try:
            ...
            ).prefetch_related("likes", "comments").select_related("creator")
        ...
        for post in posts:
            try:
                formatted_posts.append({
                    ...
                    "sentiment_keywords": self._extract_sentiment_keywords(post.content)
                })
            except Exception as e:
        ...
        for comment in comments:
            try:
                formatted_comments.append({
                    ...
                    "post_creator": getattr(comment.post.creator, 'username', 'Unknown'),
                    "created_at": comment.created_at.isoformat(),
                    "hour_of_day": comment.created_at.hour,
                    "day_of_week": comment.created_at.weekday(),
                    "sentiment_keywords": self._extract_sentiment_keywords(comment.content),
        ...
        for like in likes:
            try:
                formatted_likes.append({
                    ...
                    "post_creator": getattr(like.post.creator, 'username', 'Unknown'),
    ...
    def _extract_sentiment_keywords(self, content: str) -> Dict[str, int]:
        ...
        try:
            ...
            sentiment_patterns = {
                "positive": [r'\bhappy\b', r'\bgood\b', r'\bgreat\b', r'\bamazing\b', r'\blove\b', r'\bjoy\b'],
                "negative": [r'\bsad\b', r'\bbad\b', r'\bawful\b', r'\bterrible\b', r'\bhate\b', r'\bangry\b'],
                "anxiety": [r'\banxious\b', r'\bworried\b', r'\bstressed\b', r'\bnervous\b'],
                "support": [r'\bsupport\b', r'\bhelp\b', r'\bcare\b', r'\bunderstand\b', r'\bthere for you\b']
            }
    ...
    def _is_supportive_comment(self, content: str) -> bool:
        ...
        try:
            ...
            supportive_indicators = [
                r'\bhere for you\b', r'\byou can do it\b', r'\bproud of you\b',
                r'\bsupport you\b', r'\bbelieve in you\b', r'\byou\'re not alone\b',
                r'\bunderstand\b', r'\bcare about you\b', r'\bsending love\b'
            ]
            
            return any(re.search(pattern, content_lower) for pattern in supportive_indicators)
    ...
    def _calculate_advanced_engagement_metrics(self, interaction_data: Dict) -> Dict:
        ...
        try:
            ...
            total_engagement_received = sum(post.get("engagement_score", 0) for post in posts)
            avg_engagement_per_post = total_engagement_received / total_posts if total_posts > 0 else 0
            
            # Content quality metrics
            avg_post_length = np.mean([post.get("content_length", 0) for post in posts]) if posts else 0
            avg_comment_length = np.mean([comment.get("content_length", 0) for comment in given_comments]) if given_comments else 0
            
            # Interaction ratios
            giving_receiving_ratio = len(given_comments) / max(1, len(received_comments))
            likes_comments_ratio = len(given_likes) / max(1, len(given_comments))
            
            # Supportive behavior
            supportive_comments = sum(1 for comment in given_comments if comment.get("is_supportive", False))
            supportive_ratio = supportive_comments / len(given_comments) if given_comments else 0
            ...
            return {
                ...
                "interaction_diversity": self._calculate_interaction_diversity(interaction_data)
            }
    ...
    def _calculate_engagement_consistency(self, posts: List[Dict]) -> float:
        ...
        try:
            ...
            consistency = max(0, 1 - cv)  # Convert to 0-1 scale where 1 is most consistent
    ...
    def _analyze_content_sentiment(self, interaction_data: Dict) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "sentiment_ratio": total_positive / max(1, total_negative + total_anxiety)
            }
    ...
    def _analyze_social_network(self, interaction_data: Dict) -> Dict:
        ...
        try:
            ...
            user_interactions = defaultdict(lambda: {"comments_to": 0, "comments_from": 0, "likes_to": 0, "likes_from": 0})
            ...
            for user, interactions in user_interactions.items():
                ...
                network_analysis[user] = {
                    **interactions,
                    ...
                    "is_mutual": interactions["comments_to"] > 0 and interactions["comments_from"] > 0,
            ...
            return {
                ...
                "mutual_connections": sum(1 for data in network_analysis.values() if data["is_mutual"]),
                "top_connections": dict(top_connections),
                "network_density": len(user_interactions) / max(1, 
                    len(interaction_data.get("created_posts", [])) + 
                    len(interaction_data.get("given_comments", []))
                ),
                "average_relationship_strength": np.mean([
                    data["relationship_strength"] for data in network_analysis.values()
                ]) if network_analysis else 0
            }
    ...
    def _analyze_temporal_patterns(self, interaction_data: Dict) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "peak_hour": peak_hour,
                "peak_day": peak_day,
                "activity_consistency": self._calculate_temporal_consistency(all_activities),
                "weekend_vs_weekday_ratio": self._calculate_weekend_ratio(day_distribution)
            }
    ...
    def _calculate_temporal_consistency(self, activities: List[Dict]) -> float:
        ...
        try:
            ...
            hours = [activity["hour"] for activity in activities if activity["hour"] is not None]
            if len(hours) < 2:
                return 0.0
    ...
    def _calculate_weekend_ratio(self, day_distribution: Dict) -> float:
        ...
        try:
            weekday_activity = sum(count for day, count in day_distribution.items() if day < 5)  # Monday=0 to Friday=4
            weekend_activity = sum(count for day, count in day_distribution.items() if day >= 5)  # Saturday=5, Sunday=6
    ...
    def _perform_correlation_analysis(self, interaction_data: Dict, mood_data: List[Dict], days: int) -> Dict:
        """Perform statistical correlation analysis between social activity and mood"""
        try:
            if not mood_data:
                return {"correlation_analysis": {"available": False, "reason": "No mood data"}}
            ...
            if len(common_dates) < 3:
                return {"correlation_analysis": {"available": False, "reason": "Insufficient overlapping data"}}
            ...
            try:
                correlation_coefficient, p_value = stats.pearsonr(activity_values, mood_values)
            except:
                correlation_coefficient, p_value = 0.0, 1.0
            
            # Analyze patterns
            high_activity_moods = [mood_values[i] for i, activity in enumerate(activity_values) if activity > np.median(activity_values)]
            low_activity_moods = [mood_values[i] for i, activity in enumerate(activity_values) if activity <= np.median(activity_values)]
            ...
            return {
                "correlation_analysis": {
                    ...
                    "interpretation": self._interpret_correlation(correlation_coefficient, p_value),
                    "high_activity_avg_mood": np.mean(high_activity_moods) if high_activity_moods else 0,
                    "low_activity_avg_mood": np.mean(low_activity_moods) if low_activity_moods else 0,
            ...
            return {"correlation_analysis": {"available": False, "reason": f"Analysis error: {str(e)}"}}
...
{{
    ...
    "recommendations": [<actionable recommendations>]
        ...
        if len(prompt) > self.max_prompt_length:
            prompt = prompt[:self.max_prompt_length - 100] + "...\n\nProvide the JSON analysis."
    ...
    def _check_for_concerning_patterns(self, user, analysis: Dict):
        ...
        try:
            if analysis.get("needs_attention") and analysis.get("attention_priority") in ["medium", "high"]:
