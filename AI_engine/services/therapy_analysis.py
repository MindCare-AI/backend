...
class TherapyAnalysisService:
    ...
    def __init__(self):
        ...
        self.model = getattr(settings, 'AI_ENGINE_SETTINGS', {}).get('DEFAULT_MODEL', 'mistral')
    ...
    def analyze_for_therapy_session(self, user, days: int = 7) -> Dict[str, Any]:
        ...
        try:
            ...
            result = self._format_therapy_analysis_response(analysis, recommendation_record)
    ...
    def _collect_comprehensive_data(self, user, days: int) -> Dict[str, Any]:
        ...
        try:
            ...
            journal_data = [
                {
                    ...
                    "category": getattr(entry.category, 'name', 'general') if hasattr(entry, 'category') and entry.category else 'general',
                    "content_length": len(entry.content),
                    "emotional_indicators": self._extract_emotional_indicators(entry.content)
                }
            ...
            contextual_factors = self._get_contextual_factors(user, start_date, end_date)
    ...
    def _extract_emotional_indicators(self, content: str) -> Dict[str, int]:
        ...
        try:
            ...
            emotional_patterns = {
                "anxiety_indicators": [r'\banxious\b', r'\bworried\b', r'\bstressed\b', r'\bnervous\b', r'\bpanic\b'],
                "depression_indicators": [r'\bsad\b', r'\bdepressed\b', r'\bhopeless\b', r'\bempty\b', r'\bworthless\b'],
                "anger_indicators": [r'\bangry\b', r'\bfrustrated\b', r'\birritated\b', r'\bmad\b', r'\bfurious\b'],
                "positive_indicators": [r'\bhappy\b', r'\bjoyful\b', r'\bexcited\b', r'\bproud\b', r'\bgrateful\b'],
                "coping_indicators": [r'\bbreathing\b', r'\bmeditation\b', r'\bexercise\b', r'\btalk\b', r'\bhelp\b']
            }
    ...
    def _calculate_trend_analysis(self, mood_data: List[Dict], journal_data: List[Dict]) -> Dict:
        """Calculate trend analysis for therapy insights"""
        try:
            if not mood_data:
                return {"mood_trend": "insufficient_data", "journal_trend": "insufficient_data"}
            ...
            if journal_data:
                ...
                recent_moods = [entry["mood"] for entry in journal_data[-5:] if entry.get("mood")]
                if recent_moods:
                    positive_count = sum(1 for mood in recent_moods if mood in ["positive", "very_positive"])
                    negative_count = sum(1 for mood in recent_moods if mood in ["negative", "very_negative"])
            ...
            return {
                ...
                "data_consistency": len(mood_data) / 7 if len(mood_data) <= 7 else 1.0  # Ratio of actual to expected data points
            }
    ...
    def _get_contextual_factors(self, user, start_date, end_date) -> Dict:
        ...
        try:
            ...
            try:
                ...
                contextual_data["medication_mentions"] = len(re.findall(r'\b(medication|pill|dose|medicine)\b', all_content))
                contextual_data["sleep_mentions"] = len(re.findall(r'\b(sleep|tired|insomnia|rest)\b', all_content))
                contextual_data["social_mentions"] = len(re.findall(r'\b(friend|family|social|alone|lonely)\b', all_content))
...
"""
    ...
    def _validate_and_enhance_analysis(self, analysis: Dict) -> Dict:
        ...
        required_fields = {
            ...
            "risk_assessment": {"risk_level": "medium", "risk_factors": [], "protective_factors": []},
    ...
    def _create_fallback_analysis(self, data: Dict) -> Dict:
        ...
        return {
            ...
            "priority_issues": ["mood_stability"] if trend_analysis.get("mood_trend") == "declining" else [],
            ...
            "homework_assignments": ["Continue mood logging", "Practice deep breathing exercises"],
            "risk_assessment": {
                "risk_level": "medium" if trend_analysis.get("mood_trend") == "declining" else "low",
                "risk_factors": ["declining_mood_trend"] if trend_analysis.get("mood_trend") == "declining" else [],
                "protective_factors": ["regular_app_usage", "self_monitoring"]
            },
            "treatment_plan_updates": ["Consider increasing session frequency"] if trend_analysis.get("mood_trend") == "declining" else [],
            "next_session_timing": "1 week",
            "requires_immediate_attention": False,
    ...
    def _create_recommendation_record(self, user, analysis: Dict) -> Optional[object]:
        ...
        try:
            ...
            logger.info(f"Created therapy recommendation record {recommendation.id} for user {user.id}")
    ...
    def _check_for_urgent_insights(self, user, analysis: Dict):
        ...
        try:
            ...
            if (analysis.get("requires_immediate_attention") or 
                ...
                AIInsight.objects.create(
                    ...
                    insight_data={
                        ...
                        "immediate_actions": analysis.get("intervention_suggestions", [])
                    },
                    priority="high"
                )
    ...
    def _format_therapy_analysis_response(self, analysis: Dict, recommendation_record) -> Dict:
        """Format the final therapy analysis response"""
        return {
            "success": True,
            "recommendation_id": recommendation_record.id if recommendation_record else None,
            "analysis": {
                "focus_areas": analysis.get("focus_areas", []),
                "session_goals": analysis.get("session_goals", []),
                "therapeutic_approaches": analysis.get("therapeutic_approaches", []),
        ...
        try:
            ...
            for rec in recent_recommendations:
                insights.append({
                    ...
                    "therapeutic_approaches": rec.recommendation_data.get("therapeutic_approaches", []),
                    "current_goals": rec.recommendation_data.get("session_goals", []),
                    "created_at": rec.created_at.isoformat(),
                    "is_implemented": rec.is_implemented
                })
