...
class UserResumeService:
    ...
    def generate_user_resume(
        ...
        try:
            ...
            card_1_mental_health = self._generate_mental_health_card(
                user, dataset, period_days
            )
            card_2_behavioral = self._generate_behavioral_patterns_card(
                user, dataset, period_days
            )
            card_3_social_engagement = self._generate_social_engagement_card(
                user, dataset, period_days
            )
            card_4_progress_tracking = self._generate_progress_tracking_card(
                user, dataset, period_days
            )

            # Generate AI-driven therapist guidance
            ai_therapist_summary = self._generate_ai_therapist_summary(
                user,
                dataset,
                {
    ...
    def _generate_mental_health_card(
        self, user, dataset: Dict, period_days: int
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            latest_mood_trend = (
                MoodTrendAnalysis.objects.filter(user=user, analysis_type="weekly")
                .order_by("-created_at")
                .first()
            )
            ...
            return {
                ...
                "summary_metrics": {
                    "average_mood_score": round(
                        mood_analytics.get("average_mood", 5), 2
                    ),
                    "mood_trend": mood_analytics.get("mood_trend", "stable"),
                    "mood_volatility": round(
                        mood_analytics.get("mood_volatility", 0), 2
                    ),
                ...
                "journal_sentiment_trend": journal_analytics.get(
                    "sentiment_trend", "neutral"
                ),
                "mood_consistency_score": latest_mood_trend.consistency_score
                if latest_mood_trend
                else 0,
    ...
    def _generate_behavioral_patterns_card(
        self, user, dataset: Dict, period_days: int
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            feature_metrics = FeatureUsageMetrics.objects.filter(
                user=user, date__gte=timezone.now().date() - timedelta(days=period_days)
            ).aggregate(
            ...
            if feature_metrics["avg_sessions"] and feature_metrics["avg_sessions"] > 3:
                ...
                feature_metrics["avg_sessions"] and feature_metrics["avg_sessions"] < 1
            ):
            ...
            if activity_patterns.get("peak_usage_time"):
                behavioral_insights.append(
                    f"Most active during {activity_patterns['peak_usage_time']}"
                )
            ...
            return {
                ...
                "usage_metrics": {
                    "avg_daily_sessions": round(
                        feature_metrics["avg_sessions"] or 0, 1
                    ),
                    "avg_session_duration": round(
                        feature_metrics["total_time"] or 0, 1
                    ),
                    "engagement_score": round(
                        feature_metrics["avg_engagement"] or 0, 2
                    ),
                    "usage_trend": app_usage_trend,
                },
                "activity_patterns": {
                    "peak_usage_time": activity_patterns.get(
                        "peak_usage_time", "Unknown"
                    ),
                    "most_used_features": activity_patterns.get(
                        "most_used_features", []
                    )[:3],
                    "interaction_frequency": activity_patterns.get(
                        "interaction_frequency", "moderate"
                    ),
                },
                "behavioral_insights": behavioral_insights,
                "engagement_level": self._calculate_engagement_level(
                    feature_metrics["avg_engagement"] or 0
                ),
    ...
    def _generate_social_engagement_card(
        self, user, dataset: Dict, period_days: int
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            comm_metrics = (
                CommunicationMetrics.objects.filter(
                    user=user,
            ...
            social_engagement_level = social_analytics.get(
                "social_engagement", "moderate"
            )
            support_network_quality = social_analytics.get("support_network", "unknown")

            # Communication patterns
            communication_sentiment = communication_analytics.get(
                "sentiment_trend", "neutral"
            )
            interaction_frequency = communication_analytics.get(
                "interaction_frequency", "moderate"
            )
            ...
            return {
                ...
                "communication_patterns": {
                    ...
                    "messages_received": comm_metrics.messages_received
                    if comm_metrics
                    else 0,
                    "avg_response_time": comm_metrics.avg_response_time_minutes
                    if comm_metrics
                    else 0,
                },
                "social_insights": social_insights,
                "interaction_quality": social_analytics.get(
                    "interaction_patterns", "healthy"
                ),
                "community_involvement": social_analytics.get(
    ...
    def _generate_progress_tracking_card(
        self, user, dataset: Dict, period_days: int
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            latest_snapshot = (
                UserDataSnapshot.objects.filter(user=user)
            ...
            if latest_snapshot and previous_snapshot:
                progress_indicators = {
                    "mood_improvement": latest_snapshot.avg_mood_score
                    - previous_snapshot.avg_mood_score,
                    "engagement_change": latest_snapshot.social_engagement_score
                    - previous_snapshot.social_engagement_score,
                    "data_quality_improvement": latest_snapshot.data_quality_score
                    - previous_snapshot.data_quality_score,
                }
            ...
            therapy_readiness = self._assess_therapy_readiness(
                user, dataset, latest_snapshot
            )
            ...
            return {
                ...
                "progress_metrics": {
                    "mood_trend_direction": "improving"
                    if progress_indicators.get("mood_improvement", 0) > 0
                    else "stable",
                    "overall_progress_score": self._calculate_overall_progress_score(
                        progress_indicators
                    ),
                    "data_completeness": dataset.get("quality_metrics", {}).get(
                        "completeness", 0
                    ),
                ...
                "recent_ai_recommendations": recent_recommendations[
                    :3
                ],  # Top 3 recommendations
                "intervention_priorities": intervention_priorities,
                "therapy_readiness": therapy_readiness,
                "next_assessment_due": (timezone.now() + timedelta(days=7))
                .date()
                .isoformat(),
    ...
    def _generate_ai_therapist_summary(
        self, user, dataset: Dict, cards_data: Dict
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            therapist_guidance = self._parse_therapist_guidance(
            ...
            return {
                "ai_summary": therapist_guidance.get(
                    "summary", "AI analysis not available"
                ),
                "key_concerns": therapist_guidance.get("concerns", []),
                "therapeutic_recommendations": therapist_guidance.get(
                    "recommendations", []
                ),
    ...
    def _create_therapist_guidance_prompt(
        self, user, dataset: Dict, cards_data: Dict
    ) -> str:
    ...
    def _parse_therapist_guidance(self, ai_text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            concerns_match = re.search(
                r"KEY_CONCERNS:\s*(.*?)(?=THERAPEUTIC_RECOMMENDATIONS:|$)",
                ai_text,
                re.DOTALL | re.IGNORECASE,
            )
            if concerns_match:
            ...
            recommendations_match = re.search(
                r"THERAPEUTIC_RECOMMENDATIONS:\s*(.*?)(?=FOCUS_AREAS:|$)",
                ai_text,
                re.DOTALL | re.IGNORECASE,
            )
            ...
            focus_match = re.search(
                r"FOCUS_AREAS:\s*(.*?)(?=RISK_ASSESSMENT:|$)",
                ai_text,
                re.DOTALL | re.IGNORECASE,
            )
            if focus_match:
                ...
                guidance["focus_areas"] = [
                    f.strip("- ").strip() for f in focus_text.split("\n") if f.strip()
                ]

            # Extract risk assessment
            risk_match = re.search(
                r"RISK_ASSESSMENT:\s*(.*?)(?=INTERVENTIONS:|$)",
                ai_text,
                re.DOTALL | re.IGNORECASE,
            )
            if risk_match:
    ...
    def _calculate_social_health_score(
        self, engagement, support_network, sentiment, comm_metrics
    ) -> float:
    ...
    def _get_recent_recommendations(self, user) -> List[Dict]:
        ...
        try:
            ...
            recent_recs = TherapyRecommendation.objects.filter(user=user).order_by(
                "-created_at"
            )[:5]
    ...
    def _assess_therapy_readiness(
        self, user, dataset: Dict, snapshot
    ) -> Dict[str, Any]:
    ...
    def _generate_clinical_insights(self, dataset: Dict, cards_data: Dict) -> List[str]:
        ...
        if usage_trend == "decreasing":
            insights.append(
                "Declining app engagement may indicate decreased motivation"
            )
        ...
        if social_health < 0.4:
            insights.append(
                "Social isolation patterns detected - consider group therapy"
            )
