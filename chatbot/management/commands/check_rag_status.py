...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if not os.path.exists(chunks_dir):
            ...
            self.stdout.write(self.style.SUCCESS("YES - You need to train the RAG system"))
            return
        ...
        if not local_vector_store.loaded:
            ...
            self.stdout.write(self.style.SUCCESS("YES - You need to retrain the RAG system"))
            return
        ...
        if os.path.exists(config_path):
            with open(config_path, 'r') as f:
                config = json.load(f)
                self.stdout.write(f"✅ Vector store loaded successfully")
                self.stdout.write(f"   - CBT chunks: {len(local_vector_store.cbt_chunks)}")
                self.stdout.write(f"   - DBT chunks: {len(local_vector_store.dbt_chunks)}")
        ...
        try:
            results = local_vector_store.search_similar_chunks("anxiety and stress", limit=3)
            if results and len(results) > 0:
                self.stdout.write(f"✅ Similarity search working ({len(results)} results)")
                for i, result in enumerate(results[:2]):
                    self.stdout.write(f"   - Result {i+1}: {result['similarity']:.3f} similarity")
            else:
                self.stdout.write(self.style.WARNING("⚠️ Similarity search returned no results"))
                self.stdout.write(self.style.SUCCESS("MAYBE - Consider retraining for better results"))
                return
        except Exception as e:
            self.stdout.write(self.style.ERROR(f"❌ Similarity search failed: {str(e)}"))
            self.stdout.write(self.style.SUCCESS("YES - You need to retrain the RAG system"))
            return
            
        self.stdout.write(self.style.SUCCESS("✅ RAG system appears to be working properly"))
        self.stdout.write(self.style.SUCCESS("NO - No need to retrain unless you have new documents"))
