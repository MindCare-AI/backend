...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        rag_messages = ChatMessage.objects.filter(
            is_bot=True, metadata__rag_used=True
        ).order_by("-timestamp")[:20]

        total_responses = rag_messages.count()
        cbt_responses = rag_messages.filter(
            metadata__therapy_recommendation__approach="cbt"
        ).count()
        dbt_responses = rag_messages.filter(
            metadata__therapy_recommendation__approach="dbt"
        ).count()
        ...
        self.stdout.write(self.style.SUCCESS("✅ RAG System Performance:"))
        ...
        if avg_confidence > 0.7:
            ...
            self.stdout.write(
                self.style.ERROR("❌ LOW QUALITY responses - consider retraining")
            )
