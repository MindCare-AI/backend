...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            type=int,
            default=1000,
            help='Size of text chunks for RAG'
        )
        parser.add_argument(
            '--chunk-overlap',
            type=int,
    ...
    def handle(self, *args, **options):
        try:
            ...
            for directory in [cbt_dir, dbt_dir, indexed_dir, chunks_dir, 
            ...
            self.stdout.write(self.style.SUCCESS(f"Starting therapy RAG setup from {source_dir}"))
            
            # Check GPU availability
            gpu_info = verify_gpu_support()
            if gpu_info and gpu_info.get("using_gpu"):
                self.stdout.write(self.style.SUCCESS(f"🚀 GPU ENABLED: {gpu_info.get('details', '')}"))
                
                # Optimize GPU memory usage
                os.environ["OLLAMA_NUM_GPU"] = str(os.getenv("OLLAMA_NUM_GPU", "80"))
                self.stdout.write(self.style.SUCCESS(f"Set GPU layers to {os.environ['OLLAMA_NUM_GPU']} for maximum performance"))
            else:
                self.stdout.write(self.style.WARNING("⚠️ CPU ONLY MODE: No GPU detected or not properly configured"))
            
            # Check for PDF, DOCX, and TXT files
            cbt_files = [f for f in os.listdir(cbt_dir) if f.lower().endswith(('.pdf', '.docx', '.txt'))]
            dbt_files = [f for f in os.listdir(dbt_dir) if f.lower().endswith(('.pdf', '.docx', '.txt'))]
            
            if not cbt_files and not dbt_files:
                self.stdout.write(self.style.ERROR("No PDF, DOCX, or TXT files found in the source directories."))
                self.stdout.write(f"Please add files to {cbt_dir} and {dbt_dir}")
                return
                
            self.stdout.write(f"Found {len(cbt_files)} CBT PDFs/DOCX/TXT and {len(dbt_files)} DBT PDFs/DOCX/TXT")
                ...
                for file_name in tqdm(cbt_files, desc="CBT Files"):
                    ...
                    try:
                        ...
                        if file_name.lower().endswith('.pdf'):
                            ...
                            self.stdout.write(self.style.WARNING(f"  ⚠️ Skipping unsupported file: {file_name}"))
                            continue
                        docs = loader.load()
                        for doc in docs:
                        ...
                        self.stdout.write(f"  ✓ Processed {file_name} ({len(docs)} pages)")
            ...
            if os.path.exists(db_path) and not force:
                self.stdout.write(f"Vector store already exists at {db_path}. Use --force to recreate.")
                ...
                vectorstore = FAISS(embedding_function=embeddings, index=index, docstore=None, index_to_docstore_id=None)
            ...
            cbt_chunks = [i for i, chunk in enumerate(chunks) if chunk.metadata.get("therapy_type") == "cbt"]
            dbt_chunks = [i for i, chunk in enumerate(chunks) if chunk.metadata.get("therapy_type") == "dbt"]
            ...
            try:
                ...
                self.stdout.write(f"Sources: {[doc.metadata.get('source', 'Unknown') for doc in result['source_documents']]}")
            except Exception as e:
            ...
            self.stdout.write(f"Created {len(chunks)} chunks from {len(documents)} documents")
            self.stdout.write(f"Vector store saved to {db_path}")
            
        except Exception as e:
            logger.error(f"Error setting up therapy RAG system: {str(e)}", exc_info=True)
            self.stderr.write(self.style.ERROR(f"Error in setup_therapy_rag command: {str(e)}"))
