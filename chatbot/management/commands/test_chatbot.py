    ...
    def handle(self, *args, **kwargs):
        ...
        if use_local:
            ...
            if local_vector_store.loaded:
                doc_count = (
                    len(local_vector_store.documents)
                    if hasattr(local_vector_store, "documents")
                    else 0
                )
                cbt_count = (
                    len(local_vector_store.cbt_chunks)
                    if hasattr(local_vector_store, "cbt_chunks")
                    else 0
                )
                dbt_count = (
                    len(local_vector_store.dbt_chunks)
                    if hasattr(local_vector_store, "dbt_chunks")
                    else 0
                )
                ...
                if doc_count > 0 and (cbt_count > 0 or dbt_count > 0):
                    ...
                    self.stdout.write(
                        self.style.WARNING(
                            "⚠️ Local vector store loaded but no content found"
                        )
                    )
            else:
                self.stdout.write(self.style.ERROR("✗ Local vector store not loaded"))
                return
        else:
        ...
        if user:
            try:
                ...
                if created:
                    self.stdout.write(
                        f"Created test conversation with ID: {test_conversation_id}"
                    )
                else:
                    self.stdout.write(
                        f"Using existing test conversation with ID: {test_conversation_id}"
                    )
            except Exception as e:
                self.stdout.write(
        ...
        for i, test_case in enumerate(test_cases):
            ...
            try:
                ...
                self.stdout.write(
                    f"- Approach: {rag_response.get('recommended_approach', 'unknown')}"
                )
                self.stdout.write(
                ...
                if user and test_conversation_id:
                    chatbot_response = chatbot_service.get_response(
                        ...
                        ),  # Use the real conversation ID
                        conversation_history=[],
                    )

                    self.stdout.write(
                        f"Chatbot Response Length: {len(chatbot_response.get('content', ''))}"
                    )
                    if len(chatbot_response.get("content", "")) > 100:
                        self.stdout.write(
                            f"Preview: {chatbot_response['content'][:100]}..."
                        )
                    else:
                        self.stdout.write(
                            f"Content: {chatbot_response.get('content', '')}"
                        )
                elif user:
                    # Test with RAG only if no conversation available
                    self.stdout.write("Testing RAG service only (no conversation ID)")
                    self.stdout.write(
                        f"RAG service working: {bool(rag_response.get('recommended_approach'))}"
                    )
                else:
