...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not user:
            self.stdout.write(self.style.ERROR("No user found for testing. Creating a test user..."))
            user = User.objects.create_user(username="test_user", email="test@example.com", password="testpassword")
        
        for message in test_messages:
            # Test the enhanced crisis detection
            crisis_detection = chatbot_service._enhanced_crisis_detection(message)
            is_crisis = crisis_detection["is_crisis"] and crisis_detection["confidence"] >= chatbot_service.min_crisis_confidence
            ...
            self.stdout.write(f"  Detected as crisis: {color}{is_crisis}{Style.RESET_ALL}")
            self.stdout.write(f"  Confidence: {color}{crisis_detection['confidence']:.2f}{Style.RESET_ALL}")
            self.stdout.write(f"  Category: {color}{crisis_detection['category']}{Style.RESET_ALL}")
            ...
            if is_crisis:
                response = chatbot_service._generate_crisis_response(user, message, crisis_detection)
                self.stdout.write("\n  Crisis Response Preview:")
                self.stdout.write(f"  {color}{'-' * 40}{Style.RESET_ALL}")
                self.stdout.write(f"  {response['content'][:200]}...")
                self.stdout.write(f"  {color}{'-' * 40}{Style.RESET_ALL}")
                self.stdout.write(f"  Response Type: {response['metadata']['response_type']}")
