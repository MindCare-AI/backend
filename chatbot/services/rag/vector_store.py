...
class VectorStore:
    ...
    def __init__(self):
        self.embedding_model = getattr(settings, "RAG_SETTINGS", {}).get(
            "EMBEDDING_MODEL", "nomic-embed-text:latest"
        )
        self.embedding_dimension = getattr(settings, "RAG_SETTINGS", {}).get(
            "EMBEDDING_DIMENSION", 768
        )
        self.similarity_threshold = getattr(settings, "RAG_SETTINGS", {}).get(
            "SIMILARITY_THRESHOLD", 0.65
        )
        self.ollama_host = getattr(settings, "RAG_SETTINGS", {}).get(
