...
CHATBOT_TEST_CASES = [
    {
        ...
        "expected_keywords": ["cognitive distortion", "negative thought", "reframe", "evidence", "challenge"],
    },
    {
        ...
        "expected_keywords": ["emotion regulation", "dialectical", "mindfulness", "validation"],
    },
...
class ChatbotAccuracyTester:
        ...
        if use_rag_test_cases:
            ...
            for case in RAG_TEST_CASES:
                self.test_cases.append({
                    ...
                    "expected_keywords": self._get_keywords_for_approach(case["expected_approach"]),
                })
    ...
    def _get_keywords_for_approach(self, approach: str) -> List[str]:
        ...
        if approach.lower() == "cbt":
            return [
                "cognitive", "thought", "evidence", "challenge", "restructuring", 
                "distortion", "belief", "reframe", "thinking pattern", "negative thought",
                "cognitive distortion", "thought record", "core belief", "automatic thought",
                "evidence-based", "cognitive restructuring", "balanced thinking", "examine evidence",
                "thought challenging", "rational response", "cognitive behavioral", "cognitive therapy"
            ]
        elif approach.lower() == "dbt":
            return [
                "dialectical", "emotion regulation", "mindfulness", "distress tolerance", 
                "interpersonal effectiveness", "validation", "acceptance", "dialectical behavior",
                "wise mind", "radical acceptance", "middle path", "distress tolerance",
                "emotional awareness", "self-soothe", "linehan", "mindful observation",
                "emotional regulation", "dbt skill", "opposite action", "dialectical thinking"
            ]
        else:
            return []
    ...
    def run_tests(self, mock_user=None) -> Dict[str, Any]:
        ...
        for i, test_case in enumerate(tqdm(self.test_cases, desc="Testing chatbot")):
            ...
            try:
                ...
                actual_approach = response.get("metadata", {}).get("therapy_recommendation", {}).get("approach", "unknown")
                approach_match = actual_approach == expected_approach
                if approach_match:
                    approach_correct += 1
                
                # Check for keyword matches
                keyword_score = self._calculate_keyword_score(response["content"], expected_keywords)
    ...
    def _calculate_keyword_score(self, response_text: str, expected_keywords: List[str]) -> float:
        ...
        for keyword in expected_keywords:
            ...
            if len(keyword_parts) == 1 and len(keyword_parts[0]) > 5:
                ...
                for word in words_in_response:
                    if word.startswith(keyword_root) and keyword not in matched_keywords:
    ...
    def _save_results(self, metrics: Dict[str, Any]) -> None:
        ...
        try:
            ...
            filename = os.path.join(results_dir, f"chatbot_accuracy_results_{timestamp}.json")
    ...
    def run_benchmarks(mock_user=None) -> Dict[str, Any]:
        ...
        logger.info(f"Chatbot Accuracy Test Results:")
        ...
        logger.info(f"Average keyword score: {results['metrics']['avg_keyword_score']:.2f}")
        logger.info(f"Comprehensive score: {results['metrics']['comprehensive_score']:.2f}")
        
        return results

@pytest.mark.parametrize("query,expected_approach", [
    # core use cases
