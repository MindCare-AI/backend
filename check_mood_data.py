    ...
    for user in CustomUser.objects.all():
        ...
        if count > 0:
            ...
            for log in recent_logs:
                print(f"  - {log.logged_at.strftime('%Y-%m-%d %H:%M')} - Mood: {log.mood_rating}/10 - Activities: {log.activities}")
        else:
...
def generate_mood_trend_analysis(user, analysis_type='weekly', days_back=30):
    ...
    if mood_logs.count() < 2:
        print(f"Not enough mood data for user {user.username} (need at least 2 entries)")
        return None
