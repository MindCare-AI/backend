    ...
    for table_name, user_column in related_tables:
        try:
            with connection.cursor() as cursor:
                if user_column:
                    cursor.execute(
                        f"DELETE FROM {table_name} WHERE {user_column} = %s", [user_id]
                    )
                    deleted_counts[table_name] = cursor.rowcount
                    if cursor.rowcount > 0:
                        print(
                            f"  - Deleted {cursor.rowcount} records from {table_name}"
                        )
    ...
    if test_user:
        ...
        try:
            ...
            try:
                ...
                if "does not exist" in str(e):
                    print(
                        "Using direct SQL to delete user due to missing related tables..."
                    )
                    with connection.cursor() as cursor:
                        cursor.execute("DELETE FROM auth_user WHERE id = %s", [user_id])
                        print(
                            f"Deleted test user via SQL: {user_email} (ID: {user_id})"
                        )
                else:
                    raise e
