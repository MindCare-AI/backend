    ...
    def handle(self, *args, **options):
        ...
        if options['mode']:
            ...
            if options['mode'].lower() == 'local':
                ...
                self.stdout.write(self.style.ERROR('Invalid mode. Use "local" or "cloud"'))
        else:
            # Toggle the current mode
            if is_cloud:
                set_key(env_path, 'USE_CLOUD', 'False')
                self.stdout.write(self.style.SUCCESS('Database mode changed from CLOUD to LOCAL'))
            else:
                set_key(env_path, 'USE_CLOUD', 'True')
                self.stdout.write(self.style.SUCCESS('Database mode changed from LOCAL to CLOUD'))
