...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        run = DataCollectionRun.objects.create(
            ...
            status='running',
            metadata={
                'command': 'populate_datawarehouse',
                'options': options
            }
        )
            ...
            for user in users:
                try:
                    ...
                    if 'mood' in options['analysis_types']:
                        mood_count = self.populate_mood_analysis(user, options['days'], options['force'])
                        total_processed += mood_count
                        
                    if 'journal' in options['analysis_types']:
                        journal_count = self.populate_journal_analysis(user, options['days'], options['force'])
                        total_processed += journal_count
                        
                    if 'communication' in options['analysis_types']:
                        comm_count = self.populate_communication_analysis(user, options['days'], options['force'])
                        total_processed += comm_count
                        
                    if 'features' in options['analysis_types']:
                        feature_count = self.populate_feature_analysis(user, options['days'], options['force'])
                        total_processed += feature_count
                        
                    if 'snapshots' in options['analysis_types']:
                        snapshot_count = self.populate_user_snapshots(user, options['days'], options['force'])
    ...
    def populate_mood_analysis(self, user, days_back, force):
        ...
        if not mood_logs.exists():
            self.stdout.write(f'  No mood logs found for user {user.id} in the last {days_back} days')
            return count
        ...
        for analysis_type, period_days in analysis_periods:
            ...
            if existing and not force:
                self.stdout.write(f'  Skipping existing {analysis_type} analysis for user {user.id}')
                continue
                
            # Use logged_at instead of created_at
            period_logs = mood_logs.filter(
                logged_at__date__gte=period_start
            )
            
            if period_logs.count() < 2:
                self.stdout.write(f'  Not enough data for {analysis_type} analysis (need at least 2 entries, found {period_logs.count()})')
                continue
            ...
            self.stdout.write(f'  {action} {analysis_type} mood analysis for user {user.id}')
    ...
    def calculate_mood_trends(self, mood_logs):
        ...
        pattern_data = {
            'daily_averages': {
                date: float(np.mean(moods)) 
                for date, moods in daily_averages.items()
            },
        ...
        next_period_prediction = max(1, min(10, next_period_prediction))  # Clamp to 1-10 range
    ...
    def populate_communication_analysis(self, user, days_back, force):
        ...
        self.stdout.write(f'  Communication analysis not yet implemented for user {user.id}')
        return 0
