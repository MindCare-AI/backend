...
class AuditLog(models.Model):
    ...
    duration_ms = models.IntegerField(null=True, blank=True, help_text="Event duration in milliseconds")
    ...
    changed_fields = models.JSONField(default=list, help_text="List of fields that changed")
    
    # Additional Context
    action_details = models.JSONField(default=dict, help_text="Additional action-specific details")
    request_data = models.JSONField(null=True, blank=True, help_text="Request parameters/body")
    response_data = models.JSONField(null=True, blank=True, help_text="Response data (if applicable)")
    
    # Security and Integrity
    checksum = models.CharField(max_length=64, help_text="SHA-256 checksum for integrity verification")
    correlation_id = models.UUIDField(null=True, blank=True, help_text="Groups related events")
...
class DataChangeHistory(models.Model):
    """Detailed history of data changes for sensitive information"""
    
    id = models.BigAutoField(primary_key=True)
    audit_log = models.ForeignKey(AuditLog, on_delete=models.CASCADE, related_name='change_history')
