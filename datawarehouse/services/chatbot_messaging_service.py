...
class ChatbotMessagingCollectionService:
    ...
    def collect_chatbot_data(self, user_id: int, days_back: int = 30) -> ChatbotMessagingDataSnapshot:
        """
        ...
        try:
            ...
            # Calculate analytics
            conversation_stats = self._calculate_conversation_statistics(conversations_data)
            conversation_patterns = self._analyze_conversation_patterns(conversations_data, messages_data)
            ...
            engagement_metrics = self._analyze_engagement_metrics(conversations_data, messages_data)
            usage_patterns = self._analyze_usage_patterns(conversations_data, messages_data)
            ...
            return ChatbotMessagingDataSnapshot(
                ...
                active_conversations=sum(1 for conv in conversations_data if conv.get('is_active', False)),
                user_message_count=sum(1 for msg in messages_data if not msg.get('is_bot', False)),
                bot_message_count=sum(1 for msg in messages_data if msg.get('is_bot', False)),
    ...
    def _collect_raw_conversation_data(self, user_id: int, days_back: int) -> List[Dict]:
        ...
        try:
            ...
            conversations = ChatbotConversation.objects.filter(
                ...
                user_message_count=Count('messages', filter=Q(messages__is_bot=False)),
                bot_message_count=Count('messages', filter=Q(messages__is_bot=True))
            ).values(
                'id', 'title', 'created_at', 'last_activity', 'is_active',
                'metadata', 'message_count', 'user_message_count', 'bot_message_count'
            )
    ...
    def _collect_raw_message_data(self, user_id: int, days_back: int) -> List[Dict]:
        ...
        try:
            ...
            messages = ChatMessage.objects.filter(
                ...
                'id', 'conversation_id', 'content', 'is_bot', 'timestamp',
                'message_type', 'metadata', 'sender_id', 'parent_message_id'
            )
    ...
    def _calculate_conversation_statistics(self, conversations_data: List[Dict]) -> Dict[str, Any]:
        ...
        try:
            ...
            message_counts = [conv.get('message_count', 0) for conv in conversations_data]
            ...
            total_user_messages = sum(conv.get('user_message_count', 0) for conv in conversations_data)
            total_bot_messages = sum(conv.get('bot_message_count', 0) for conv in conversations_data)
            
            user_bot_ratio = total_user_messages / total_bot_messages if total_bot_messages > 0 else 0
            ...
            return {
                ...
                'average_conversation_duration_hours': round(avg_conversation_duration, 2),
                ...
                'message_distribution': {
                    ...
                    'median_messages': sorted(message_counts)[len(message_counts)//2] if message_counts else 0
                }
            }
    ...
    def _analyze_conversation_patterns(self, conversations_data: List[Dict], messages_data: List[Dict]) -> Dict[str, Any]:
        """Analyze patterns in conversation behavior"""
        try:
            if not conversations_data:
                return {'creation_patterns': {}, 'engagement_patterns': {}, 'lifecycle_patterns': {}}
            ...
            most_active_hour = max(creation_hours.items(), key=lambda x: x[1]) if creation_hours else (0, 0)
            most_active_day = max(creation_days.items(), key=lambda x: x[1]) if creation_days else ('Unknown', 0)
            ...
            for conv_id, messages in conv_message_map.items():
                if len(messages) >= 2:
                    ...
                    session_length = (last_msg - first_msg).total_seconds() / 60  # minutes
                    avg_session_lengths.append(session_length)
            
            avg_session_length = sum(avg_session_lengths) / len(avg_session_lengths) if avg_session_lengths else 0
            ...
            return {
                ...
                'engagement_patterns': {
                    ...
                    'conversations_with_multiple_messages': len([conv for conv in conversations_data if conv.get('message_count', 0) > 1]),
                    'single_message_conversations': len([conv for conv in conversations_data if conv.get('message_count', 0) == 1])
                },
                'lifecycle_patterns': {
                    'conversations_started': len(conversations_data),
                    'active_conversations': sum(1 for conv in conversations_data if conv.get('is_active', False)),
                    'retention_rate': sum(1 for conv in conversations_data if conv.get('is_active', False)) / len(conversations_data) if conversations_data else 0
                }
            }
    ...
    def _calculate_message_statistics(self, messages_data: List[Dict]) -> Dict[str, Any]:
        ...
        try:
            total_messages = len(messages_data)
            user_messages = [msg for msg in messages_data if not msg.get('is_bot', False)]
            bot_messages = [msg for msg in messages_data if msg.get('is_bot', False)]
            
            # Content length analysis
            user_content_lengths = [len(msg.get('content', '')) for msg in user_messages]
            bot_content_lengths = [len(msg.get('content', '')) for msg in bot_messages]
            
            avg_user_message_length = sum(user_content_lengths) / len(user_content_lengths) if user_content_lengths else 0
            avg_bot_message_length = sum(bot_content_lengths) / len(bot_content_lengths) if bot_content_lengths else 0
            ...
            messages_with_parent = sum(1 for msg in messages_data if msg.get('parent_message_id'))
            
            return {
                'total_messages': total_messages,
                'user_messages': len(user_messages),
                'bot_messages': len(bot_messages),
                'user_message_percentage': (len(user_messages) / total_messages * 100) if total_messages > 0 else 0,
                'bot_message_percentage': (len(bot_messages) / total_messages * 100) if total_messages > 0 else 0,
                ...
                'content_length_distribution': {
                    'user_messages': {
                        'min_length': min(user_content_lengths) if user_content_lengths else 0,
                        'max_length': max(user_content_lengths) if user_content_lengths else 0,
                        'median_length': sorted(user_content_lengths)[len(user_content_lengths)//2] if user_content_lengths else 0
                    },
                    'bot_messages': {
                        'min_length': min(bot_content_lengths) if bot_content_lengths else 0,
                        'max_length': max(bot_content_lengths) if bot_content_lengths else 0,
                        'median_length': sorted(bot_content_lengths)[len(bot_content_lengths)//2] if bot_content_lengths else 0
                    }
                }
            }
    ...
    def _analyze_response_patterns(self, messages_data: List[Dict]) -> Dict[str, Any]:
        ...
        try:
            if not messages_data:
                return {'response_times': {}, 'conversation_flow': {}, 'interaction_patterns': {}}
            ...
            most_common_pattern = max(pattern_counts.items(), key=lambda x: x[1]) if pattern_counts else ('No patterns', 0)
            
            return {
                'response_times': {
                    'average_response_time_seconds': round(avg_response_time, 2),
                    'total_responses_analyzed': len(response_times),
                    'fastest_response_seconds': min(response_times) if response_times else 0,
                    'slowest_response_seconds': max(response_times) if response_times else 0
                },
                'conversation_flow': {
                ...
                'interaction_patterns': {
                    ...
                    'average_messages_per_conversation': sum(len(msgs) for msgs in conv_messages.values()) / len(conv_messages) if conv_messages else 0,
                    'conversations_with_back_and_forth': len([msgs for msgs in conv_messages.values() if len(msgs) >= 4])
                }
            }
    ...
    def _analyze_engagement_metrics(self, conversations_data: List[Dict], messages_data: List[Dict]) -> Dict[str, Any]:
        """Analyze user engagement with the chatbot"""
        try:
            if not conversations_data:
                return {'overall_engagement': {}, 'session_metrics': {}, 'retention_metrics': {}}
            ...
            recent_conversations = [
                ...
                if conv.get('last_activity') and 
                (now - (conv['last_activity'] if isinstance(conv['last_activity'], datetime) 
                       else datetime.fromisoformat(conv['last_activity'].replace('Z', '+00:00')))).days <= 7
            ]
            
            # Active vs inactive
            active_conversations = [conv for conv in conversations_data if conv.get('is_active', False)]
            ...
            return {
                'overall_engagement': {
                    ...
                    'average_user_messages_per_conversation': len(user_messages) / total_conversations if total_conversations > 0 else 0,
                    'engagement_rate': len(user_messages) / total_messages if total_messages > 0 else 0
                },
                'session_metrics': {
                    'recent_conversations_7_days': len(recent_conversations),
                    'active_conversations': len(active_conversations),
                    'inactive_conversations': total_conversations - len(active_conversations),
                    'activity_retention_rate': len(active_conversations) / total_conversations if total_conversations > 0 else 0
                },
                'retention_metrics': {
                    'high_engagement_conversations': len(high_engagement_conversations),
                    'high_engagement_rate': len(high_engagement_conversations) / total_conversations if total_conversations > 0 else 0,
                    'average_conversation_depth': sum(conv.get('message_count', 0) for conv in conversations_data) / total_conversations if total_conversations > 0 else 0
                }
            }
    ...
    def _analyze_usage_patterns(self, conversations_data: List[Dict], messages_data: List[Dict]) -> Dict[str, Any]:
        """Analyze usage patterns over time"""
        try:
            if not messages_data:
                return {'temporal_patterns': {}, 'frequency_patterns': {}, 'usage_trends': {}}
            ...
            avg_weekly_messages = sum(weekly_usage.values()) / weeks_with_usage if weeks_with_usage > 0 else 0
            ...
            return {
                ...
                'frequency_patterns': {
                    ...
                    'most_active_week': max(weekly_usage.items(), key=lambda x: x[1])[0] if weekly_usage else 'No data',
                    'weekly_usage_distribution': weekly_usage
                },
                'usage_trends': {
                    'total_active_weeks': weeks_with_usage,
                    'consistency_score': min(1.0, weeks_with_usage / 4),  # Normalized to 4 weeks
                    'usage_variance': self._calculate_variance(list(weekly_usage.values())) if weekly_usage else 0
                }
            }
    ...
    def _analyze_content_patterns(self, messages_data: List[Dict]) -> Dict[str, Any]:
        ...
        try:
            if not messages_data:
                return {'content_themes': {}, 'message_characteristics': {}, 'communication_style': {}}
            ...
            mental_health_keywords = [
                'anxiety', 'depression', 'stress', 'worry', 'sad', 'happy', 'angry',
                'therapy', 'counseling', 'help', 'support', 'feel', 'emotion',
                'mood', 'mental', 'health', 'cope', 'struggle', 'better', 'worse'
            ]
            ...
            avg_user_word_count = sum(avg_user_words) / len(avg_user_words) if avg_user_words else 0
            avg_bot_word_count = sum(avg_bot_words) / len(avg_bot_words) if avg_bot_words else 0
            ...
            return {
                ...
                'message_characteristics': {
                    ...
                    'question_rate': question_count / len(user_messages) if user_messages else 0
                },
                'communication_style': {
                    'user_message_length_category': self._categorize_message_length(avg_user_word_count),
                    'engagement_level': self._calculate_engagement_level(question_count, exclamation_count, len(user_messages)),
    ...
    def _analyze_topic_trends(self, messages_data: List[Dict]) -> Dict[str, Any]:
        ...
        try:
            if not messages_data:
                return {'trending_topics': {}, 'topic_evolution': {}, 'seasonal_patterns': {}}
            
            # Define topic categories for mental health
            topic_categories = {
                'emotional_state': ['happy', 'sad', 'angry', 'frustrated', 'excited', 'calm', 'anxious'],
                'coping_strategies': ['meditation', 'exercise', 'breathing', 'therapy', 'medication', 'counseling'],
                'life_events': ['work', 'family', 'relationship', 'school', 'job', 'stress', 'change'],
                'symptoms': ['sleep', 'appetite', 'energy', 'concentration', 'mood', 'panic', 'worry'],
                'support_seeking': ['help', 'support', 'advice', 'guidance', 'talk', 'listen', 'understand']
            }
            ...
            return {
                ...
                'topic_evolution': {
                    ...
                    'most_discussed_category': max(
                        [(cat, sum(monthly_topics.get(month, {}).get(cat, 0) for month in monthly_topics)) 
                         for cat in topic_categories.keys()], 
                        key=lambda x: x[1]
                    )[0] if monthly_topics else 'No data',
                    'topic_diversity_score': len(set().union(*(monthly_topics.get(month, {}).keys() for month in monthly_topics)))
                },
                'seasonal_patterns': {
                    'active_months': len(monthly_topics),
                    'consistent_topics': [
                        topic for topic in topic_categories.keys() 
                        if sum(1 for month in monthly_topics.values() if topic in month) >= len(monthly_topics) * 0.5
                    ] if monthly_topics else []
                }
            }
    ...
    def _calculate_engagement_level(self, questions: int, exclamations: int, total_messages: int) -> str:
    ...
    def _assess_formality(self, user_messages: List[Dict]) -> str:
        ...
        formal_indicators = ['please', 'thank you', 'appreciate', 'kindly', 'however', 'furthermore']
    ...
    def _create_empty_snapshot(self) -> ChatbotMessagingDataSnapshot:
        ...
        return ChatbotMessagingDataSnapshot(
            ...
            conversation_patterns={'creation_patterns': {}, 'engagement_patterns': {}, 'lifecycle_patterns': {}},
            message_statistics=self._empty_message_stats(),
            response_patterns={'response_times': {}, 'conversation_flow': {}, 'interaction_patterns': {}},
            engagement_metrics={'overall_engagement': {}, 'session_metrics': {}, 'retention_metrics': {}},
            usage_patterns={'temporal_patterns': {}, 'frequency_patterns': {}, 'usage_trends': {}},
            content_analysis={'content_themes': {}, 'message_characteristics': {}, 'communication_style': {}},
            topic_trends={'trending_topics': {}, 'topic_evolution': {}, 'seasonal_patterns': {}},
            performance_metrics={'processing_time_seconds': 0, 'conversations_processed': 0, 'messages_processed': 0, 'timestamp': timezone.now().isoformat()},
            timestamp=timezone.now()
        )
    ...
    def _empty_conversation_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'message_distribution': {'min_messages': 0, 'max_messages': 0, 'median_messages': 0}
        }
