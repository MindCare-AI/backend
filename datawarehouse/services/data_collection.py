...
class UserDataSnapshot:
    ...
    def __post_init__(self):
        ...
        for field in [
            ...
            "mood_journal_snapshot",
            "therapist_session_snapshot",
            "feeds_snapshot",
        ]:
            if getattr(self, field) is None:
                setattr(self, field, {})
...
class DataCollectionService:
    ...
    def _initialize_specialized_services(self):
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    "MoodTrackingCollectionService not found, using legacy mood collection"
                )
            ...
            try:
                ...
                logger.warning(
                    "JournalingCollectionService not found, using legacy journal collection"
                )
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=self.config.max_workers) as executor:
                ...
                if self.feeds_service:
                    futures[
                        executor.submit(self._collect_with_feeds_service, user, days)
                    ] = "feeds_service"

                if self.mood_service:
                    futures[
                        executor.submit(self._collect_with_mood_service, user, days)
                    ] = "mood_service"

                if self.journal_service:
                    futures[
                        executor.submit(self._collect_with_journal_service, user, days)
                    ] = "journal_service"
            ...
            snapshot = UserDataSnapshot(
                ...
                therapist_session_snapshot=specialized_snapshots.get(
                    "therapist_service", {}
                ),
    ...
    def _collect_messaging_data(
        ...
        try:
            ...
            ).values("id", "content", "sender_id", "timestamp", "conversation_id")
            ...
            analysis = {
                ...
                "avg_message_length": float(df["content"].str.len().mean())
                if not df.empty
                else 0.0,
    ...
    def _collect_analytics_data(
        ...
        try:
            ...
            analysis = {
                "count": len(df),
                "total_sessions": int(df["sessions_count"].sum())
                if not df.empty
                else 0,
                "avg_session_time": float(df["total_time_spent"].mean())
                if not df.empty
                else 0.0,
    ...
    def _collect_social_data(
        ...
        try:
            ...
            posts = Post.objects.filter(
                author=user, created_at__range=(start_date, end_date)
            ...
            analysis = {
                ...
                "social_engagement": self._calculate_social_engagement(
    ...
    def _collect_with_therapist_service(self, user, days: int) -> Dict[str, Any]:
        ...
        try:
            if self.therapist_service:
                snapshot = self.therapist_service.collect_therapist_session_data(
                    user, days
                )
    ...
    def _calculate_mood_improvement(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if (
            df.empty
            or "mood_before" not in df.columns
            or "mood_after" not in df.columns
        ):
        ...
        return {
            "avg_improvement": float(mood_diffs.mean())
            if not mood_diffs.empty
            else 0.0,
    ...
    def _analyze_communication_frequency(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_messages_per_day": len(df) / df["timestamp"].dt.date.nunique()
            if not df.empty
            else 0.0,
        }
    ...
    def _analyze_message_sentiment(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        return {"avg_sentiment": float(np.mean(sentiments)) if sentiments else 0.0}
    ...
    def _analyze_appointment_frequency(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        return {
            "avg_per_week": float(weekly_counts.mean())
    ...
    def _calculate_social_engagement(self, posts, comments, likes) -> Dict[str, Any]:
        ...
        return {
            ...
            "posts_ratio": total_posts / (total_posts + total_comments + total_likes)
            if (total_posts + total_comments + total_likes) > 0
            else 0.0,
            "engagement_level": "high"
            if (total_posts + total_comments + total_likes) > 20
            else "medium"
            if (total_posts + total_comments + total_likes) > 5
            else "low",
        }
