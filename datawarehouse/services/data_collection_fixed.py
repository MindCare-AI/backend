...
from django.db.models import Q, Count, Avg, Sum, F, Value, Case, When
...
class DataCollectionService:
    ...
    def collect_user_data(self, user_id: int, days: int = 30) -> UserDataSnapshot:
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=self.config.max_workers) as executor:
                ...
                futures = {
                    executor.submit(self._collect_mood_data, user, start_date, end_date): 'mood',
                    executor.submit(self._collect_journal_data, user, start_date, end_date): 'journal',
                    ...
                    executor.submit(self._collect_social_data, user, start_date, end_date): 'social'
                }
    ...
    def _collect_messaging_data(self, user, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
        ...
        try:
            ...
            communication_stats = self._analyze_communication_patterns(one_to_one_df, group_df)
            
            return {
                'one_to_one_messages': one_to_one_df.to_dict('records') if not one_to_one_df.empty else [],
                'group_messages': group_df.to_dict('records') if not group_df.empty else [],
                'statistics': communication_stats,
                'data_quality': {
                    'message_completeness': (len(one_to_one_df) + len(group_df)) / max(1, (end_date - start_date).days)
                }
            }
    ...
    def _collect_appointment_data(self, user, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
        ...
        try:
            ...
            ).values('id', 'scheduled_time', 'status', 'duration', 'appointment_type', 'notes')
            ...
            appointment_stats = {
                ...
                'completed_appointments': len(df[df['status'] == 'completed']) if 'status' in df else 0,
                'cancelled_appointments': len(df[df['status'] == 'cancelled']) if 'status' in df else 0,
                'avg_duration': df['duration'].mean() if 'duration' in df else 0,
                'appointment_frequency': len(df) / max(1, (end_date - start_date).days / 7)  # per week
            }
    ...
    def _collect_notification_data(self, user, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
        ...
        try:
            ...
            notification_stats = {
                ...
                'unread_notifications': len(df[df['unread'] == True]) if 'unread' in df else 0,
                'read_rate': 1 - (len(df[df['unread'] == True]) / len(df)) if 'unread' in df and len(df) > 0 else 0,
                'notification_types': df['verb'].value_counts().to_dict() if 'verb' in df else {}
            }
            ...
            return {
                ...
                'data_quality': {
                    'engagement_rate': notification_stats['read_rate']
                }
            }
    ...
    def _collect_social_data(self, user, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
        ...
        try:
            ...
            social_stats = {
                ...
                'social_engagement_score': self._calculate_social_engagement(posts_df, comments_df, likes_df)
            }
    ...
    def _analyze_activity_patterns(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'activity_frequency': len(all_activities) / len(df) if len(df) > 0 else 0
            }
    ...
    def _analyze_journal_sentiment(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            positive_words = ['happy', 'good', 'great', 'wonderful', 'joy', 'excited', 'grateful']
            negative_words = ['sad', 'bad', 'terrible', 'awful', 'angry', 'frustrated', 'worried']
    ...
    def _extract_journal_topics(self, df: pd.DataFrame) -> List[str]:
        ...
        try:
            ...
            common_topics = ['work', 'family', 'friends', 'health', 'exercise', 'sleep', 'therapy', 'medication']
            found_topics = []
    ...
    def _analyze_communication_patterns(self, one_to_one_df: pd.DataFrame, group_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            stats = {
                ...
                'message_ratio': len(one_to_one_df) / max(1, len(group_df)) if len(group_df) > 0 else len(one_to_one_df)
            }
            ...
            if not combined_df.empty and 'timestamp' in combined_df:
                ...
                stats['most_active_hour'] = int(combined_df['hour'].mode().iloc[0]) if not combined_df['hour'].mode().empty else None
                stats['hourly_distribution'] = combined_df['hour'].value_counts().to_dict()
    ...
    def _calculate_social_engagement(self, posts_df: pd.DataFrame, comments_df: pd.DataFrame, likes_df: pd.DataFrame) -> float:
        ...
        try:
            ...
            score = (len(posts_df) * posts_weight + 
    ...
    def _get_empty_messaging_data(self) -> Dict[str, Any]:
        return {
            'one_to_one_messages': [],
            'group_messages': [],
            'statistics': {'one_to_one_messages': 0, 'group_messages': 0, 'total_messages': 0},
