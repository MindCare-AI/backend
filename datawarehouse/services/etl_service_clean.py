...
class ETLService:
    ...
    def _process_user_data(self, user, since=None):
        """
        ...
        try:
            ...
            cache_key = (
                f"user_data_{user.id}_{int(time.time() // 3600)}"  # Hour-based cache
            )
    ...
    def _transform_user_data(self, df, user):
        """
        ...
        try:
            ...
            if (
                "mood_data" in transformed.columns
                and not transformed["mood_data"].isna().all()
            ):
                ...
                if isinstance(mood_data, dict) and mood_data.get("entries"):
                    ...
                    if mood_scores:
                        ...
                        transformed["mood_trend"] = self._calculate_mood_trend(
                            mood_scores
                        )
            ...
            if (
                "journal_data" in transformed.columns
                and not transformed["journal_data"].isna().all()
            ):
                ...
                if isinstance(journal_data, dict) and journal_data.get("entries"):
                    ...
                    entry_lengths = [
                        len(entry.get("content", "")) for entry in journal_entries
                    ]
                    transformed["avg_journal_length"] = (
                        np.mean(entry_lengths) if entry_lengths else 0
                    )

            # Process messaging data
            if (
                "messaging_data" in transformed.columns
                and not transformed["messaging_data"].isna().all()
            ):
                messaging_data = transformed["messaging_data"].iloc[0]
                if isinstance(messaging_data, dict):
                    transformed["messages_sent"] = messaging_data.get(
                        "messages_sent", 0
                    )
                    transformed["messages_received"] = messaging_data.get(
                        "messages_received", 0
                    )
                    transformed["avg_response_time"] = messaging_data.get(
                        "avg_response_time", 0
                    )

            # Add data quality indicators
            transformed["data_completeness_score"] = self._calculate_completeness_score(
                transformed
            )
            ...
            self.logger.error(
                "Error transforming user data", user_id=user.id, exc_info=True
            )
            raise
    ...
    def _create_user_snapshot(self, user, transformed_data):
        ...
        try:
            ...
            if "journal_entries_count" in row and pd.notna(
                row["journal_entries_count"]
            ):
            ...
            # Update metadata
            if "data_completeness_score" in row and pd.notna(
            ...
            self.logger.debug(
                f"{'Created' if created else 'Updated'} snapshot for user {user.id}"
            )
            return snapshot

        except Exception:
            self.logger.error(
                f"Error creating snapshot for user {user.id}", exc_info=True
            )
            return None

    def _find_recently_active_users(self, since):
        """
        Find users with recent activity
        ...
        try:
            ...
            # Check for users with data collection runs since the specified time
