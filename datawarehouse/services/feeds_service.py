...
class FeedsCollectionService:
    ...
    def collect_feeds_data(self, user=None, days: int = 30) -> FeedsDataSnapshot:
        ...
        try:
            ...
            snapshot = FeedsDataSnapshot(
                ...
                therapeutic_content_analysis=therapeutic_analysis['therapeutic_content'],
                peer_support_metrics=therapeutic_analysis['peer_support'],
                crisis_intervention_indicators=therapeutic_analysis['crisis_indicators'],
            ...
            logger.info(f"Feeds data collection completed in {time.time() - start_time:.2f}s")
            return snapshot
    ...
    def _collect_raw_feeds_data(self, user, days: int) -> Dict[str, Any]:
        ...
        try:
            ...
            # Collect reactions
            reactions = Reaction.objects.filter(reaction_filter).select_related(
                'user', 'content_type'
            ).order_by('-created_at')
    ...
    def _calculate_content_metrics(self, posts: List, comments: List) -> Dict[str, Any]:
        ...
        try:
            ...
            post_lengths = [len(post.content) for post in posts if hasattr(post, 'content')]
            post_word_counts = [len(post.content.split()) for post in posts if hasattr(post, 'content')]
            
            # Comment content analysis
            comment_lengths = [len(comment.content) for comment in comments if hasattr(comment, 'content')]
            comment_word_counts = [len(comment.content.split()) for comment in comments if hasattr(comment, 'content')]
            
            # Media analysis
            posts_with_media = sum(1 for post in posts if hasattr(post, 'media_files') and post.media_files.exists())
            
            # Link analysis
            posts_with_links = sum(1 for post in posts if hasattr(post, 'link_url') and post.link_url)
            
            return {
                'avg_post_length': statistics.mean(post_lengths) if post_lengths else 0,
                'median_post_length': statistics.median(post_lengths) if post_lengths else 0,
                'avg_post_word_count': statistics.mean(post_word_counts) if post_word_counts else 0,
                'avg_comment_length': statistics.mean(comment_lengths) if comment_lengths else 0,
                'median_comment_length': statistics.median(comment_lengths) if comment_lengths else 0,
                'avg_comment_word_count': statistics.mean(comment_word_counts) if comment_word_counts else 0,
                'posts_with_media_percentage': (posts_with_media / max(1, len(posts))) * 100,
                'posts_with_links_percentage': (posts_with_links / max(1, len(posts))) * 100,
                'long_posts': sum(1 for length in post_lengths if length > 500),
                'short_posts': sum(1 for length in post_lengths if length < 50),
                'detailed_comments': sum(1 for length in comment_lengths if length > 100),
    ...
    def _analyze_topic_distribution(self, posts: List) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'most_popular_topic': topic_counts.most_common(1)[0][0] if topic_counts else None,
                'posts_with_topics_percentage': (posts_with_topics / max(1, total_posts)) * 100,
    ...
    def _analyze_post_types(self, posts: List) -> Dict[str, Any]:
        ...
        try:
            ...
            poll_posts = [post for post in posts if hasattr(post, 'post_type') and post.post_type == 'poll']
            ...
            return {
                ...
                'media_posts': type_counts.get('image', 0) + type_counts.get('video', 0),
    ...
    def _analyze_engagement_patterns(self, raw_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            engagement_metrics = self._calculate_engagement_metrics(posts, comments, reactions)
            
            # Analyze interaction patterns
            interaction_patterns = self._analyze_interaction_patterns(posts, comments, reactions)
            
            # Calculate community health metrics
            community_health = self._calculate_community_health_metrics(posts, comments, reactions)
    ...
    def _calculate_engagement_metrics(self, posts: List, comments: List, reactions: List) -> Dict[str, Any]:
        ...
        try:
            ...
            avg_reactions_per_post = statistics.mean(post_reaction_counts) if post_reaction_counts else 0
            avg_comments_per_post = statistics.mean(post_comment_counts) if post_comment_counts else 0
            avg_views_per_post = statistics.mean(post_view_counts) if post_view_counts else 0
            
            # Engagement distribution
            high_engagement_posts = sum(1 for count in post_reaction_counts if count > avg_reactions_per_post * 2)
            low_engagement_posts = sum(1 for count in post_reaction_counts if count == 0)
            ...
            return {
                ...
                'avg_reactions_per_comment': statistics.mean(comment_reaction_counts) if comment_reaction_counts else 0,
                'high_engagement_posts': high_engagement_posts,
                'low_engagement_posts': low_engagement_posts,
                'engagement_variance': statistics.variance(post_reaction_counts) if len(post_reaction_counts) > 1 else 0,
                'most_engaged_post_reactions': max(post_reaction_counts) if post_reaction_counts else 0,
                'most_commented_post': max(post_comment_counts) if post_comment_counts else 0,
                'total_engagement_score': sum(post_reaction_counts) + sum(post_comment_counts)
            }
    ...
    def _analyze_interaction_patterns(self, posts: List, comments: List, reactions: List) -> Dict[str, Any]:
        ...
        try:
            ...
            total_users = len(set(list(user_post_counts.keys()) + 
                                list(user_comment_counts.keys()) + 
                                list(user_reaction_counts.keys())))
            
            # Power user analysis
            power_users = self._identify_power_users(user_post_counts, user_comment_counts, user_reaction_counts)
            
            # Interaction diversity
            interaction_diversity = self._calculate_interaction_diversity(posts, comments, reactions)
            
            return {
                'total_active_users': total_users,
                'avg_posts_per_user': statistics.mean(list(user_post_counts.values())) if user_post_counts else 0,
                'avg_comments_per_user': statistics.mean(list(user_comment_counts.values())) if user_comment_counts else 0,
                'avg_reactions_per_user': statistics.mean(list(user_reaction_counts.values())) if user_reaction_counts else 0,
                'power_users': power_users,
                'interaction_diversity': interaction_diversity,
                'lurkers': total_users - len(user_post_counts) - len(user_comment_counts),
    ...
    def _identify_power_users(self, post_counts: Counter, comment_counts: Counter, reaction_counts: Counter) -> Dict[str, Any]:
        ...
        try:
            ...
            power_users = [user_id for user_id, score in user_scores.items() if score > power_threshold]
            moderate_users = [user_id for user_id, score in user_scores.items() if moderate_threshold < score <= power_threshold]
            casual_users = [user_id for user_id, score in user_scores.items() if score <= moderate_threshold]
    ...
    def _calculate_interaction_diversity(self, posts: List, comments: List, reactions: List) -> Dict[str, Any]:
        """Calculate diversity of interactions"""
        try:
            # User interaction matrix
            user_interactions = defaultdict(lambda: {'posts': 0, 'comments': 0, 'reactions': 0})
            ...
            return {
                'avg_interaction_diversity': statistics.mean(diversity_scores) if diversity_scores else 0,
                'users_with_diverse_interactions': sum(1 for score in diversity_scores if score > 0.5),
                'single_interaction_users': sum(1 for interactions in user_interactions.values() 
                                              if sum(1 for count in interactions.values() if count > 0) == 1),
                'multi_interaction_users': sum(1 for interactions in user_interactions.values() 
                                             if sum(1 for count in interactions.values() if count > 0) > 1)
            }
    ...
    def _calculate_community_health_metrics(self, posts: List, comments: List, reactions: List) -> Dict[str, Any]:
        """Calculate community health and toxicity metrics"""
        try:
            # Positive interaction indicators
            positive_reactions = sum(1 for reaction in reactions 
                                   if hasattr(reaction, 'reaction_type') and 
                                   reaction.reaction_type in ['love', 'support', 'celebrate'])
            ...
            user_activity_days = self._calculate_user_activity_spread(posts, comments, reactions)
            
            return {
                'positive_reaction_percentage': (positive_reactions / max(1, total_reactions)) * 100,
                'supportive_content_percentage': supportive_content['supportive_percentage'],
                ...
                'community_engagement_score': self._calculate_community_engagement_score(
                    positive_reactions, total_reactions, supportive_content, response_rate
                ),
    ...
    def _analyze_supportive_content(self, posts: List, comments: List) -> Dict[str, Any]:
        """Analyze supportive and helpful content"""
        try:
            supportive_keywords = [
                r'\bsupport\b', r'\bhelp\b', r'\bcare\b', r'\bunderstand\b',
                r'\bthere for you\b', r'\byou.*not.*alone\b', r'\bproud of you\b',
                r'\byou.*can.*do.*it\b', r'\bbelieve.*in.*you\b', r'\bsending.*love\b'
            ]
            
            crisis_keywords = [
                r'\bcrisis\b', r'\bemergency\b', r'\bsuicidal\b', r'\bhurt.*myself\b',
                r'\bend.*it.*all\b', r'\bhotline\b', r'\b911\b', r'\bgive.*up\b'
            ]
            
            helpful_keywords = [
                r'\btip\b', r'\badvice\b', r'\brecommend\b', r'\bsuggestion\b',
                r'\btry.*this\b', r'\bworked.*for.*me\b', r'\bresource\b'
            ]
            ...
            for post in posts:
                if hasattr(post, 'content'):
                    ...
                    if any(re.search(keyword, content_lower) for keyword in supportive_keywords):
                        supportive_posts += 1
                    
                    if any(re.search(keyword, content_lower) for keyword in crisis_keywords):
                        crisis_support += 1
                    
                    if any(re.search(keyword, content_lower) for keyword in helpful_keywords):
                        helpful_content += 1
            ...
            return {
                ...
                'supportive_percentage': (supportive_content_total / max(1, total_content)) * 100,
                'crisis_support': crisis_support,
                'helpful_indicators': helpful_content,
                'supportive_to_total_ratio': supportive_content_total / max(1, total_content)
            }
    ...
    def _calculate_user_activity_spread(self, posts: List, comments: List, reactions: List) -> Dict[str, Any]:
        ...
        try:
            ...
            for comment in comments:
                if hasattr(comment, 'author') and hasattr(comment, 'created_at'):
                    user_activity_dates[comment.author.id].add(comment.created_at.date())
            
            for reaction in reactions:
                if hasattr(reaction, 'user') and hasattr(reaction, 'created_at'):
                    user_activity_dates[reaction.user.id].add(reaction.created_at.date())
            ...
            return {
                'avg_activity_days': statistics.mean(activity_day_counts) if activity_day_counts else 0,
                ...
                'most_active_user_days': max(activity_day_counts) if activity_day_counts else 0
            }
    ...
    def _calculate_community_engagement_score(self, positive_reactions: int, total_reactions: int, 
                                            supportive_content: Dict, response_rate: float) -> float:
        ...
        try:
            ...
            normalized_response_rate = min(1.0, response_rate / 2.0)  # Normalize response rate
    ...
    def _analyze_social_dynamics(self, raw_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Network analysis
            network_analysis = self._calculate_network_metrics(posts, comments, reactions)
    ...
    def _calculate_network_metrics(self, posts: List, comments: List, reactions: List) -> Dict[str, Any]:
        ...
        try:
            ...
            for comment in comments:
                if hasattr(comment, 'author') and hasattr(comment, 'post') and hasattr(comment.post, 'author'):
            ...
            highly_connected = sum(1 for count in connection_counts if count > avg_connections * 2)
    ...
    def _calculate_network_density(self, user_connections: Dict) -> float:
        ...
        try:
            ...
            actual_connections = sum(len(connections) for connections in user_connections.values()) / 2
    ...
    def _analyze_support_interactions(self, posts: List, comments: List) -> Dict[str, Any]:
        ...
        try:
            ...
            request_keywords = [r'\bneed.*help\b', r'\bstruggling\b', r'\badvice\b', r'\bwhat.*should.*i.*do\b']
            offer_keywords = [r'\bhere.*for.*you\b', r'\bcan.*help\b', r'\bpm.*me\b', r'\breachout\b']
            advice_keywords = [r'\btry\b', r'\bsuggestion\b', r'\brecommend\b', r'\bworked.*for.*me\b']
            encouragement_keywords = [r'\byou.*can.*do.*it\b', r'\bproud.*of.*you\b', r'\bkeep.*going\b', r'\bstay.*strong\b']
            ...
            for content in all_content:
                ...
                if any(re.search(keyword, content_lower) for keyword in request_keywords):
                    support_requests += 1
                if any(re.search(keyword, content_lower) for keyword in offer_keywords):
                    support_offers += 1
                if any(re.search(keyword, content_lower) for keyword in advice_keywords):
                    peer_advice += 1
                if any(re.search(keyword, content_lower) for keyword in encouragement_keywords):
                    encouragement += 1
            ...
            return {
                ...
                'support_request_percentage': (support_requests / max(1, total_content)) * 100,
                'support_offer_percentage': (support_offers / max(1, total_content)) * 100,
                'peer_support_ratio': support_offers / max(1, support_requests),
                'total_supportive_interactions': support_offers + peer_advice + encouragement
            }
    ...
    def _analyze_mental_health_content(self, posts: List, comments: List) -> Dict[str, Any]:
        """Analyze mental health related content"""
        try:
            mental_health_topics = {
                'anxiety': [r'\banxiety\b', r'\banxious\b', r'\bpanic\b', r'\bworried\b'],
                'depression': [r'\bdepression\b', r'\bdepressed\b', r'\bsad\b', r'\bhopeless\b'],
                'stress': [r'\bstress\b', r'\bstressed\b', r'\boverwhelmed\b', r'\bpressure\b'],
                'therapy': [r'\btherapy\b', r'\btherapist\b', r'\bcounseling\b', r'\bsession\b'],
                'medication': [r'\bmedication\b', r'\bpills\b', r'\bantidepressant\b', r'\bprescription\b'],
                'coping': [r'\bcoping\b', r'\bstrategy\b', r'\bmindfulness\b', r'\bmeditation\b'],
                'recovery': [r'\brecovery\b', r'\bhealing\b', r'\bgetting.*better\b', r'\bprogress\b']
            }
            ...
            return {
                ...
                'mental_health_content_percentage': (mental_health_content_total / max(1, total_content)) * 100,
                'most_discussed_topic': max(topic_counts.items(), key=lambda x: x[1])[0] if topic_counts else None,
                'therapeutic_content_instances': topic_counts.get('therapy', 0) + topic_counts.get('coping', 0),
                'crisis_related_content': topic_counts.get('depression', 0) + topic_counts.get('anxiety', 0),
                'recovery_focused_content': topic_counts.get('recovery', 0) + topic_counts.get('coping', 0)
            }
    ...
    def _analyze_temporal_patterns(self, raw_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Activity patterns
            activity_patterns = self._calculate_activity_patterns(posts, comments, reactions)
    ...
    def _calculate_activity_patterns(self, posts: List, comments: List, reactions: List) -> Dict[str, Any]:
    ...
    def _analyze_peak_activity(self, posts: List, comments: List, reactions: List) -> Dict[str, Any]:
        ...
        try:
            ...
            low_activity_days = sum(1 for count in activity_counts if count < avg_daily_activity * 0.5)
            high_activity_days = sum(1 for count in activity_counts if count > avg_daily_activity * 2)
            ...
            return {
                ...
                'activity_variance': statistics.variance(activity_counts) if len(activity_counts) > 1 else 0,
                'consistent_activity_days': sum(1 for count in activity_counts 
    ...
    def _analyze_therapeutic_value(self, raw_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            therapeutic_content = self._analyze_therapeutic_content_value(posts, comments)
            
            # Peer support metrics
            peer_support = self._calculate_peer_support_metrics(posts, comments)
            
            # Crisis intervention indicators
            crisis_indicators = self._analyze_crisis_intervention_indicators(posts, comments)
    ...
    def _analyze_therapeutic_content_value(self, posts: List, comments: List) -> Dict[str, Any]:
        """Analyze therapeutic value of content"""
        try:
            therapeutic_indicators = {
                'insight': [r'\brealized\b', r'\bunderstood\b', r'\binsight\b', r'\blearned\b'],
                'gratitude': [r'\bthankful\b', r'\bgrateful\b', r'\bappreciate\b', r'\bblessed\b'],
                'goal_setting': [r'\bgoal\b', r'\bplan\b', r'\btarget\b', r'\baim\b'],
                'self_reflection': [r'\breflect\b', r'\bthinking.*about\b', r'\blooking.*back\b'],
                'progress': [r'\bprogress\b', r'\bimprovement\b', r'\bbetter\b', r'\bgrowth\b'],
                'coping_strategies': [r'\bcoping\b', r'\bstrategy\b', r'\btechnique\b', r'\bmethod\b']
            }
            ...
            return {
                ...
                'therapeutic_content_percentage': (therapeutic_content_total / max(1, total_content)) * 100,
                'most_therapeutic_category': max(content_scores.items(), key=lambda x: x[1])[0] if content_scores else None,
    ...
    def _calculate_peer_support_metrics(self, posts: List, comments: List) -> Dict[str, Any]:
        """Calculate peer support effectiveness metrics"""
        try:
            # Analyze supportive responses to distress posts
            distress_keywords = [r'\bupset\b', r'\bsad\b', r'\bworried\b', r'\bstressed\b', r'\bstruggling\b']
            support_keywords = [r'\bhere.*for.*you\b', r'\bsupport\b', r'\bhelp\b', r'\bcare\b']
            ...
            for post in posts:
                if hasattr(post, 'content'):
                    content_lower = post.content.lower()
                    if any(re.search(keyword, content_lower) for keyword in distress_keywords):
            ...
            for post in distress_posts:
                if hasattr(post, 'comments'):
                    for comment in post.comments.all():
                        if hasattr(comment, 'content'):
                            content_lower = comment.content.lower()
                            if any(re.search(keyword, content_lower) for keyword in support_keywords):
            ...
            return {
                ...
                'peer_support_effectiveness': min(1.0, response_rate / 2.0),  # Normalized score
                'unaddressed_distress_posts': max(0, distress_post_count - supportive_responses)
            }
    ...
    def _analyze_crisis_intervention_indicators(self, posts: List, comments: List) -> Dict[str, Any]:
        """Analyze crisis intervention needs and responses"""
        try:
            crisis_keywords = [
                r'\bsuicidal\b', r'\bend.*it.*all\b', r'\bhurt.*myself\b', r'\bcrisis\b',
                r'\bemergency\b', r'\bcan.*t.*go.*on\b', r'\bgive.*up\b'
            ]
            
            intervention_keywords = [
                r'\bhotline\b', r'\b911\b', r'\bget.*help\b', r'\btalk.*to.*someone\b',
                r'\btherapist\b', r'\bcounselor\b', r'\bemergency.*room\b'
            ]
            ...
            for content in all_content:
                if any(re.search(keyword, content) for keyword in intervention_keywords):
            ...
            return {
                ...
                'crisis_intervention_ratio': intervention_responses / max(1, total_crisis_content),
                'crisis_content_percentage': (total_crisis_content / max(1, len(all_content))) * 100,
                'intervention_effectiveness': intervention_responses / max(1, total_crisis_content)
            }
    ...
    def _calculate_data_quality_score(self, raw_data: Dict[str, Any]) -> float:
        ...
        try:
            ...
            complete_posts = sum(1 for post in posts if hasattr(post, 'content') and post.content and 
                               hasattr(post, 'author') and post.author)
            ...
            avg_content_length = statistics.mean(post_lengths + comment_lengths) if (post_lengths + comment_lengths) else 0
            richness_score = min(1.0, avg_content_length / 200)  # Normalize to 200 chars
            
            # Engagement quality (content with reactions/comments)
            engaged_posts = sum(1 for post in posts if 
                              (hasattr(post, 'reactions') and post.reactions.count() > 0) or
                              (hasattr(post, 'comments') and post.comments.count() > 0))
            engagement_score = engaged_posts / max(1, len(posts))
            
            # Overall quality score
            overall_score = (completeness_score * 0.4 + richness_score * 0.3 + engagement_score * 0.3)
