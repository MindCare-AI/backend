...
class JournalCollectionService:
    ...
    def collect_journal_data(
        ...
        try:
            ...
            logger.info(
                "Journal data collection completed",
                user_id=user.id,
            ...
            logger.error(
                "Error collecting journal data", error=str(exc), user_id=user.id
            )
            return self._get_empty_journal_analysis()
    ...
    def _analyze_sentiment(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "sentiment_consistency": float(np.std(sentiment_scores))
        ...
        try:
            ...
            return {
                ...
                "most_discussed_topic": max(
                    topic_mentions.items(), key=lambda x: x[1], default=(None, 0)
                )[0],
        ...
        try:
            ...
            return {
                ...
                "most_active_hour": max(
                    hour_distribution.items(), key=lambda x: x[1], default=(None, 0)
                )[0],
                "most_active_day": max(
                    dow_distribution.items(), key=lambda x: x[1], default=(None, 0)
                )[0],
                "current_writing_streak": writing_streak,
                "writing_regularity_score": self._calculate_regularity_score(
                    daily_counts
                ),
            }
    ...
    def _track_emotions(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
                ...
                emotion_timeline.append(
                    {
                        ...
                        "dominant_emotion": max(
                            [
                                (e, sum(1 for k in emotions[e] if k in content_lower))
            ...
            return {
                ...
                "dominant_emotion": max(
                    emotion_counts.items(), key=lambda x: x[1], default=(None, 0)
                )[0],
                "emotional_diversity": len(
                    [e for e, c in emotion_counts.items() if c > 0]
                ),
                "emotion_timeline": emotion_timeline[-10:],  # Last 10 entries
                "emotional_balance_score": self._calculate_emotional_balance(
                    emotion_counts
                ),
            }
    ...
    def _analyze_therapeutic_themes(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
                        ...
                        "primary_theme": max(
                            [
                                (
                                    ...
                                    sum(
                                        1
                                        for k in self.therapeutic_themes[t]
            ...
            therapeutic_balance = (positive_mentions - negative_mentions) / max(
                1, positive_mentions + negative_mentions
            )

            return {
                "theme_counts": theme_counts,
                "primary_theme": max(
                    theme_counts.items(), key=lambda x: x[1], default=(None, 0)
                )[0],
    ...
    def _calculate_writing_streak(self, df: pd.DataFrame) -> int:
        ...
        try:
            ...
            writing_dates = sorted(
                pd.to_datetime(df["created_at"]).dt.date.unique(), reverse=True
            )
    ...
    def _calculate_journal_quality_score(self, df: pd.DataFrame) -> float:
        ...
        try:
            ...
            if "content" in df:
                word_counts = [
                    len(str(content).split()) for content in df["content"].dropna()
                ]
                if word_counts:
                    avg_words = np.mean(word_counts)
                    # Optimal range: 50-500 words
                    length_score = (
                        min(1.0, max(0.0, (avg_words - 10) / 40))
                        if avg_words < 50
                        else min(1.0, max(0.5, 1 - (avg_words - 500) / 1000))
                    )
            ...
            if "created_at" in df:
                ...
                daily_counts = df.groupby("date").size()
                frequency_score = min(
                    1.0, len(daily_counts) / 7
                )  # Writing at least weekly
    ...
    def get_journal_insights(self, user, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            if writing_freq.get("current_writing_streak", 0) > 7:
                insights.append(
                    f"Great job! You have a {writing_freq['current_writing_streak']}-day writing streak."
                )
            elif writing_freq.get("avg_entries_per_day", 0) < 0.3:
                recommendations.append(
                    "Try to write in your journal more regularly for better mental health tracking"
                )
            ...
            if sentiment.get("sentiment_trend") == "predominantly_positive":
                insights.append(
                    "Your journal entries show a predominantly positive emotional tone"
                )
            elif sentiment.get("sentiment_trend") == "predominantly_negative":
                insights.append(
                    "Your recent journal entries show some concerning patterns. Consider discussing with your therapist"
                )
                recommendations.append(
                    "Focus on writing about positive experiences and gratitude"
                )
            ...
            if themes.get("therapeutic_balance_score", 0) > 0.2:
                insights.append(
                    "Your journal shows good balance between processing challenges and celebrating positives"
                )
            elif (
                themes.get("therapy_engagement_indicators", {}).get(
                    "mentions_therapy", 0
                )
                > 0
            ):
                insights.append(
                    "You're actively engaging with therapeutic concepts in your writing"
                )

            # Mood improvement insights
            if journal_result.mood_improvement > 0.5:
                insights.append(
                    "Your journal writing sessions are consistently improving your mood"
                )
                recommendations.append(
                ...
                recommendations.append(
                    "Consider discussing your journaling topics with your therapist"
                )
            ...
            if journal_result.quality_score > 0.8:
                ...
                recommendations.append(
                    "Try to write longer, more detailed entries for better therapeutic benefit"
                )
            ...
            logger.error(
                "Error generating journal insights", error=str(exc), user_id=user.id
            )
