...
class JournalingCollectionService:
    ...
    def collect_journaling_data(
        self, user_id: int, days: int = 30
    ) -> JournalingDataSnapshot:
        """
        ...
        logger.info(
            f"Starting journaling data collection for user {user_id}, {days} days"
        )
        ...
        try:
            ...
            snapshot = JournalingDataSnapshot(
                ...
                raw_entries=journal_data.to_dict("records")
            ...
            logger.error(
                f"Journaling data collection failed for user {user_id}: {str(exc)}"
            )
            self.collection_stats["error_rate"] += 1
            raise

    def _collect_raw_journal_data(
    ...
    def _calculate_writing_statistics(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            statistics = {
                ...
                "content_length_std": np.std(content_lengths)
                if len(content_lengths) > 1
                else 0,
                "writing_consistency": {
                    "entries_per_week": total_entries / max(1, len(df) / 7)
                    if len(df) > 0
                    else 0,
                    "avg_days_between_entries": self._calculate_avg_days_between_entries(
                        df
                    ),
                },
            }
            ...
            if content_lengths:
                statistics["writing_depth_category"] = self._categorize_writing_depth(
                    np.mean(content_lengths)
                )
    ...
    def _analyze_sentiment(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            for _, row in df.iterrows():
                ...
                positive_count = sum(
                    1 for word in self.positive_words if word in content
                )
                negative_count = sum(
                    1 for word in self.negative_words if word in content
                )
                ...
                if total_sentiment_words > 0:
                    ...
                    intensity = (
                        total_sentiment_words / len(content.split())
                        if content.split()
                        else 0
                    )
                else:
            ...
            sentiment_progression = self._analyze_sentiment_progression(
                df, sentiment_scores
            )
            ...
            return {
                ...
                "most_positive_entry_index": int(np.argmax(sentiment_scores))
                if sentiment_scores
                else None,
                "most_negative_entry_index": int(np.argmin(sentiment_scores))
                if sentiment_scores
                else None,
            }
    ...
    def _analyze_topics(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            filtered_words = [
                word for word in all_words if word not in stop_words and len(word) > 3
            ]
            ...
            return {
                ...
                "most_common_topics": sorted(
                    topic_percentages.items(), key=lambda x: x[1], reverse=True
                )[:5],
                "entries_per_topic": {
                    topic: count
                    for topic, count in topic_occurrences.items()
                ...
                "topic_diversity": len(
                    [topic for topic, count in topic_occurrences.items() if count > 0]
                ),
                "avg_topics_per_entry": np.mean(
                    [len(topics) for topics in entry_topics]
                )
                if entry_topics
                else 0,
            }
    ...
    def _analyze_mood_correlation(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            analysis = {
                ...
                "mood_improvement_rate": len(mood_changes[mood_changes > 0])
                / len(mood_changes)
                * 100
                if len(mood_changes) > 0
                else 0,
                "significant_improvements": len(mood_changes[mood_changes >= 2])
                / len(mood_changes)
                * 100
                if len(mood_changes) > 0
                else 0,
                "mood_decline_rate": len(mood_changes[mood_changes < 0])
                / len(mood_changes)
                * 100
                if len(mood_changes) > 0
                else 0,
    ...
    def _extract_emotional_insights(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            emotional_distribution = {
                theme: (count / total_emotional_mentions) * 100
                if total_emotional_mentions > 0
                else 0
            ...
            return {
                ...
                "dominant_emotions": sorted(
                    emotional_distribution.items(), key=lambda x: x[1], reverse=True
                )[:3],
                "emotional_diversity": len(
                    [theme for theme, count in theme_counts.items() if count > 0]
                ),
    ...
    def _analyze_sentiment_progression(
        self, df: pd.DataFrame, sentiment_scores: List[float]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "recent_trend": np.mean(sentiment_scores[-5:])
                if len(sentiment_scores) >= 5
                else np.mean(sentiment_scores),
            }
    ...
    def _calculate_emotional_balance(self, theme_counts: Dict[str, int]) -> float:
        ...
        try:
            positive_emotions = (
                theme_counts.get("happiness", 0)
                + theme_counts.get("gratitude", 0)
                + theme_counts.get("hope", 0)
            )
            negative_emotions = (
                theme_counts.get("anxiety", 0)
                + theme_counts.get("depression", 0)
                + theme_counts.get("anger", 0)
            )
    ...
    def clear_cache(self, user_id: Optional[int] = None):
        ...
        if user_id:
            ...
            cache.delete_many(
                [key for key in cache.keys() if key.startswith("journaling_data:")]
            )
