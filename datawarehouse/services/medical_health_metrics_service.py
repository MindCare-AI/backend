...
class MedicalHealthMetricsCollectionService:
    ...
    def collect_medical_health_data(
        self, user_id: int, days_back: int = 90
    ) -> MedicalHealthDataSnapshot:
        """
        ...
        try:
            ...
            logger.error(
                f"Error collecting medical/health data for user {user_id}: {str(e)}"
            )
            return self._create_empty_snapshot()
    ...
    def _collect_raw_health_metrics(
        self, patient_profile: PatientProfile, days_back: int
    ) -> List[Dict]:
        ...
        try:
            ...
            metrics = (
                HealthMetric.objects.filter(
                    patient=patient_profile, timestamp__gte=cutoff_date
                )
    ...
    def _collect_raw_medical_history(
        self, patient_profile: PatientProfile
    ) -> List[Dict]:
        """Collect raw medical history data from the database"""
        try:
            history_entries = (
                MedicalHistoryEntry.objects.filter(patient=patient_profile)
                .values("id", "title", "description", "date_occurred", "is_chronic")
    ...
    def _calculate_health_metrics_statistics(
        self, health_metrics_data: List[Dict]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for metric_type, metrics in metrics_by_type.items():
                type_analysis[metric_type] = self._analyze_metric_type(
                    metrics, metric_type
                )
            ...
            most_tracked_metric = (
                max(metric_frequency.items(), key=lambda x: x[1])
                if metric_frequency
                else ("No data", 0)
            )
    ...
    def _analyze_blood_pressure(
        self, values: List[Dict], timestamps: List[datetime]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            high_bp_readings = sum(
                1 for v in values if v["systolic"] >= 140 or v["diastolic"] >= 90
            )
            elevated_bp_readings = sum(
                1 for v in values if 120 <= v["systolic"] < 140 and v["diastolic"] < 90
            )
            normal_bp_readings = sum(
                1 for v in values if v["systolic"] < 120 and v["diastolic"] < 80
            )
            ...
            return {
                ...
                "percentage_normal": (normal_bp_readings / len(values) * 100)
                if values
                else 0,
                "percentage_elevated": (elevated_bp_readings / len(values) * 100)
                if values
                else 0,
                "percentage_high": (high_bp_readings / len(values) * 100)
                if values
                else 0,
        ...
        try:
            ...
            if len(values) >= 2:
                ...
                weight_change_percentage = (
                    (weight_change / first_weight * 100) if first_weight > 0 else 0
                )
            else:
                weight_change = 0
    ...
    def _analyze_heart_rate(
        ...
        try:
            ...
            return {
                ...
                "percentage_normal": (normal_hr_readings / len(values) * 100)
                if values
                else 0,
                "percentage_high": (high_hr_readings / len(values) * 100)
                if values
                else 0,
                "percentage_low": (low_hr_readings / len(values) * 100)
                if values
                else 0,
                "trend": self._calculate_simple_trend(values),
    ...
    def _analyze_metric_trends(self, health_metrics_data: List[Dict]) -> Dict[str, Any]:
        ...
        try:
            ...
            measurement_frequency = self._analyze_measurement_frequency(
                health_metrics_data
            )
            ...
            return {
                ...
                "trend_analysis": {
                    ...
                    "data_consistency_trend": self._analyze_data_consistency_trend(
                        health_metrics_data
                    ),
                    "trend_reliability_score": self._calculate_trend_reliability(
                        trend_analysis
                    ),
                },
            }
    ...
    def _calculate_medical_history_statistics(
        self, medical_history_data: List[Dict]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            chronic_conditions = [
                ...
                if entry.get("is_chronic", False)
            ]
            acute_conditions = [
                entry
                for entry in medical_history_data
                if not entry.get("is_chronic", False)
            ]
            ...
            recent_entries = [
                ...
                and (now - entry["date_occurred"]).days <= 365
            ]
            ...
            # Get top keywords
            top_keywords = sorted(
                condition_keywords.items(), key=lambda x: x[1], reverse=True
            )[:10]
            ...
            return {
                ...
                "chronic_percentage": (len(chronic_conditions) / total_entries * 100)
                if total_entries > 0
                else 0,
                "recent_entries_last_year": len(recent_entries),
                "average_entries_per_year": self._calculate_avg_entries_per_year(
                    medical_history_data
                ),
                "top_condition_keywords": dict(top_keywords),
                "oldest_entry_date": min(
                    entry["date_occurred"]
                    for entry in medical_history_data
                    if entry.get("date_occurred")
                ),
                "newest_entry_date": max(
                    entry["date_occurred"]
                    for entry in medical_history_data
                    if entry.get("date_occurred")
                ),
                "medical_complexity_score": self._calculate_medical_complexity(
                    medical_history_data
                ),
            }
        ...
        try:
            ...
            return {
                ...
                "temporal_patterns": {
                    ...
                    "total_years_with_entries": len(
                        set(chronic_over_time.keys()) | set(acute_over_time.keys())
                    ),
                },
                "severity_patterns": {
                    "chronic_condition_burden": len(
                        [e for e in medical_history_data if e.get("is_chronic")]
                    ),
                    "condition_recurrence": self._analyze_condition_recurrence(
                        medical_history_data
                    ),
                    "most_affected_system": max(
    ...
    def _assess_risk_indicators(
        self, health_metrics_data: List[Dict], medical_history_data: List[Dict]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if "blood_pressure" in metrics_by_type:
                bp_risk = self._assess_blood_pressure_risk(
                    metrics_by_type["blood_pressure"]
                )
        ...
        try:
            ...
            if health_metrics_data:
                patterns["measurement_patterns"] = {
                    ...
                    "measurement_gaps": self._identify_measurement_gaps(
                        health_metrics_data
                    ),
                }
            ...
            patterns["health_trajectory"] = {
                "improvement_indicators": self._identify_improvement_indicators(
                    health_metrics_data
                ),
                "decline_indicators": self._identify_decline_indicators(
                    health_metrics_data
                ),
            ...
            if len(health_metrics_data) > 10:  # Need sufficient data for correlation
                patterns["correlation_analysis"] = self._analyze_metric_correlations(
                    health_metrics_data
                )
    ...
    def _analyze_temporal_patterns(
        ...
        try:
            ...
            # Find patterns
            peak_measurement_hour = (
                max(hourly_measurements.items(), key=lambda x: x[1])
                if hourly_measurements
                else (0, 0)
            )
            peak_measurement_day = (
                max(daily_measurements.items(), key=lambda x: x[1])
            ...
            return {
                ...
                "monthly_patterns": {
                    ...
                    "most_active_month": max(
                        monthly_measurements.items(), key=lambda x: x[1]
                    )[0]
                    if monthly_measurements
                    else "No data",
                },
            }
    ...
    def _calculate_compliance_metrics(
        self, health_metrics_data: List[Dict], days_back: int
    ) -> Dict[str, Any]:
        """Calculate compliance and adherence metrics"""
        try:
            if not health_metrics_data:
                return {
                    "measurement_compliance": 0,
                    "consistency_score": 0,
                    "adherence_patterns": {},
                }
            ...
            for metric_type, metrics in metrics_by_type.items():
                if metric_type in recommended_frequency:
                    expected_measurements = (
                        recommended_frequency[metric_type] * weeks_in_period
                    )
                    actual_measurements = len(metrics)
                    compliance_rate = (
                        min(1.0, actual_measurements / expected_measurements)
            ...
            overall_compliance = (
                sum(comp["compliance_rate"] for comp in compliance_by_type.values())
                / len(compliance_by_type)
                if compliance_by_type
                else 0
            )

            # Consistency analysis
            consistency_score = self._calculate_measurement_consistency(
                health_metrics_data
            )
            ...
            return {
                ...
                "compliance_level": self._categorize_compliance_level(
                    overall_compliance
                ),
                "improvement_suggestions": self._generate_compliance_suggestions(
                    compliance_by_type
                ),
            }
    ...
    def _calculate_weight_stability(self, weights: List[float]) -> float:
        ...
        stability = 1.0 - min(
            1.0, weight_range / (avg_weight * 0.1)
        )  # 10% range threshold
    ...
    def _calculate_hr_variability(self, hr_values: List[float]) -> float:
        ...
        differences = [
            abs(hr_values[i] - hr_values[i - 1]) for i in range(1, len(hr_values))
        ]
    ...
    def _calculate_data_quality_score(self, health_metrics_data: List[Dict]) -> float:
        ...
        complete_entries = sum(
            1 for metric in health_metrics_data if metric.get("value")
        )
    ...
    def _create_empty_snapshot(self) -> MedicalHealthDataSnapshot:
        ...
        return MedicalHealthDataSnapshot(
            ...
            performance_metrics={
                ...
                "health_metrics_processed": 0,
                "medical_history_entries_processed": 0,
                "timestamp": timezone.now().isoformat(),
            },
            timestamp=timezone.now(),
        )
