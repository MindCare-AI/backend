...
class MessagingCollectionService:
    ...
    def __init__(self):
        ...
        self.therapeutic_keywords = {
            ...
            'medication': ['medication', 'pills', 'prescription', 'dosage', 'side effects'],
            'crisis': ['suicide', 'kill myself', 'end it all', 'hurt myself', 'can\'t go on']
        }
    ...
    def collect_messaging_data(self, patient_id: int, days: int = 30) -> MessagingAnalysisResult:
        """
        ...
        """
        try:
            self.logger.info(f"Starting messaging data collection for patient {patient_id}")
            ...
            self.logger.info(f"Messaging data collection completed for patient {patient_id}")
            return result
            
        except Exception as e:
            self.logger.error(f"Error collecting messaging data for patient {patient_id}: {str(e)}")
            raise
    
    def _analyze_one_to_one_messages(self, patient_id: int, start_date: datetime, 
                                   end_date: datetime, result: MessagingAnalysisResult):
            ...
            for conversation in conversations:
                ...
                if messages.exists():
                    ...
                    for i, msg in enumerate(patient_messages):
                        ...
                        if prev_other_msg:
                            response_time = (msg.timestamp - prev_other_msg.timestamp).total_seconds() / 3600
                    ...
                    metrics.therapeutic_keywords = self._extract_therapeutic_keywords(all_content)
                    metrics.sentiment_score = self._calculate_sentiment_score(all_content)
                    ...
                    if metrics.last_activity and metrics.last_activity > timezone.now() - timedelta(days=7):
    ...
    def _analyze_group_messages(self, patient_id: int, start_date: datetime, 
                              end_date: datetime, result: MessagingAnalysisResult):
            ...
            for conversation in conversations:
                ...
                if messages.exists():
                    ...
                    if metrics.message_count > 0:
                        participation_ratio = patient_messages.count() / metrics.message_count
                        metrics.engagement_score = min(participation_ratio * 5, 5.0)  # Scale to 5
    ...
    def _analyze_chatbot_messages(self, patient_id: int, start_date: datetime, 
                                end_date: datetime, result: MessagingAnalysisResult):
        ...
        try:
                ...
                if messages.exists():
                    ...
                    for msg in user_messages:
                        if msg.content:
                            ...
                            if any(word in content_lower for word in ['help', 'support']):
                                query_counter['help_requests'] += 1
                            elif any(word in content_lower for word in ['mood', 'feeling']):
                                query_counter['mood_inquiries'] += 1
                            elif any(word in content_lower for word in ['medication', 'prescription']):
                    ...
                    if user_messages.count() > 0 and bot_messages.count() > 0:
                        ...
                        satisfaction_keywords = ['thank', 'helpful', 'better', 'good', 'useful']
                        satisfaction_score = sum(1 for word in satisfaction_keywords 
                                               if word in all_content.lower()) / len(satisfaction_keywords)
            ...
            if effectiveness_scores:
                result.chatbot_effectiveness = sum(effectiveness_scores) / len(effectiveness_scores)
    ...
    def _analyze_communication_patterns(self, result: MessagingAnalysisResult):
        ...
        try:
            if result.total_conversations > 0:
                result.avg_messages_per_conversation = result.total_messages / result.total_conversations
    ...
    def _detect_crisis_indicators(self, result: MessagingAnalysisResult):
        ...
        try:
            for conv_id, metrics in result.conversation_metrics.items():
                ...
                crisis_found = any(keyword in ' '.join(metrics.therapeutic_keywords).lower() 
                                 for keyword in self.crisis_keywords)
                
                if crisis_found or metrics.sentiment_score < -0.7:
                    indicator = {
                        'conversation_id': conv_id,
                        'type': 'crisis_language' if crisis_found else 'severe_negative_sentiment',
    ...
    def _analyze_support_network(self, result: MessagingAnalysisResult):
        ...
        try:
            ...
            if active_connections > 0:
                ...
                result.support_network_strength = min((active_connections * 0.3 + avg_engagement * 0.7), 5.0)
        ...
        try:
            ...
            if result.total_messages > 0:
                if result.avg_messages_per_conversation < 5:
                    insights.append("Low message volume per conversation may indicate limited engagement")
                    recommendations.append("Encourage deeper conversations with support network")
                elif result.avg_messages_per_conversation > 20:
                    insights.append("High engagement in conversations shows active communication")
            
            # Sentiment insights
            if result.overall_sentiment < -0.3:
                insights.append("Overall communication sentiment is negative, indicating potential distress")
                recommendations.append("Consider increasing positive social interactions")
            elif result.overall_sentiment > 0.3:
                insights.append("Positive communication patterns indicate good emotional state")
            ...
            if result.support_network_strength < 2.0:
                ...
                recommendations.append("Consider joining support groups or increasing social activities")
            elif result.support_network_strength > 4.0:
                insights.append("Strong support network provides good emotional foundation")
            
            # Chatbot effectiveness
            if result.chatbot_interactions > 0:
                if result.chatbot_effectiveness < 0.3:
                    insights.append("Low chatbot engagement suggests need for improved AI interactions")
                    recommendations.append("Review chatbot conversation flows and responses")
                elif result.chatbot_effectiveness > 0.7:
                    insights.append("High chatbot effectiveness shows good AI support utilization")
            
            # Crisis indicators
            if result.crisis_indicators:
                insights.append(f"Detected {len(result.crisis_indicators)} potential crisis indicators")
                recommendations.append("Immediate follow-up with mental health professional recommended")
            
            # Response time patterns
            if result.response_time_patterns.get('average', 0) > 24:
                insights.append("Delayed response times may indicate reduced social engagement")
                recommendations.append("Encourage more timely communication with support network")
    ...
    def _calculate_sentiment_score(self, content: str) -> float:
        ...
        positive_words = ['happy', 'good', 'better', 'great', 'wonderful', 'amazing', 'love', 'joy', 'excited', 'grateful']
        negative_words = ['sad', 'bad', 'worse', 'terrible', 'awful', 'hate', 'angry', 'frustrated', 'depressed', 'anxious']
    ...
    def _calculate_engagement_score(self, metrics: ConversationMetrics) -> float:
        ...
        if metrics.avg_response_time > 0:
            response_score = max(0, 2.0 - (metrics.avg_response_time / 12))  # 12 hours baseline
            score += response_score
        
        # Therapeutic keyword presence indicates deeper engagement
        if metrics.therapeutic_keywords:
            score += min(len(metrics.therapeutic_keywords) / 5, 1.0)  # Max 1 point for therapeutic content
