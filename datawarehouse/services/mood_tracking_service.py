...
class MoodTrackingCollectionService:
    ...
    def _collect_raw_mood_data(self, user, start_date: datetime, end_date: datetime) -> pd.DataFrame:
    ...
    def _calculate_mood_statistics(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            statistics = {
                ...
                'std_deviation': float(mood_scores.std()) if len(mood_scores) > 1 else 0.0,
    ...
    def _analyze_mood_trends(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            daily_moods = df.groupby('date')['mood_score'].agg(['mean', 'count']).reset_index()
            ...
            if len(x) > 1:
                ...
                r_value = correlation_matrix[0, 1] if not np.isnan(correlation_matrix[0, 1]) else 0
            ...
            return {
                ...
                'volatility': float(daily_moods['mean'].std()) if len(daily_moods) > 1 else 0.0
            }
    ...
    def _analyze_activity_patterns(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'activity_diversity_score': len(set(all_activities)) / len(all_activities) if all_activities else 0
            }
    ...
    def _analyze_sleep_patterns(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            sleep_stats = {
                ...
                'sleep_consistency': float(sleep_hours.std()) if len(sleep_hours) > 1 else 0.0,
                'optimal_sleep_range': [7, 9],  # Standard recommendation
            }
            
            # Analyze sleep-mood correlation
            if len(sleep_data) > 5:
                correlation = np.corrcoef(sleep_hours, mood_scores)[0, 1] if len(sleep_hours) > 1 else 0
                sleep_stats['sleep_mood_correlation'] = float(correlation) if not np.isnan(correlation) else 0.0
    ...
    def _analyze_energy_patterns(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            energy_stats = {
                ...
                'energy_consistency': float(energy_levels.std()) if len(energy_levels) > 1 else 0.0,
                'energy_distribution': energy_levels.value_counts().to_dict()
            }
            
            # Energy-mood correlation
            if len(energy_data) > 5:
                correlation = np.corrcoef(energy_levels, mood_scores)[0, 1] if len(energy_levels) > 1 else 0
                energy_stats['energy_mood_correlation'] = float(correlation) if not np.isnan(correlation) else 0.0
    ...
    def _analyze_stress_patterns(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            stress_stats = {
                ...
                'stress_variability': float(stress_levels.std()) if len(stress_levels) > 1 else 0.0,
                'stress_distribution': stress_levels.value_counts().to_dict()
            }
            
            # Stress-mood correlation (should be negative)
            if len(stress_data) > 5:
                correlation = np.corrcoef(stress_levels, mood_scores)[0, 1] if len(stress_levels) > 1 else 0
                stress_stats['stress_mood_correlation'] = float(correlation) if not np.isnan(correlation) else 0.0
            ...
            high_stress_days = len(stress_data[stress_data['stress_level'] >= high_stress_threshold])
            stress_stats['high_stress_days'] = high_stress_days
            stress_stats['high_stress_percentage'] = (high_stress_days / len(stress_data)) * 100 if len(stress_data) > 0 else 0
    ...
    def clear_cache(self, user_id: Optional[int] = None):
        ...
        if user_id:
            ...
            cache.delete_many([key for key in cache.keys() if key.startswith("mood_data:")])
