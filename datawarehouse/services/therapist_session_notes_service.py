...
class TherapistSessionNotesCollectionService:
    ...
    def collect_therapist_session_data(
        self, user=None, days: int = 30
    ) -> TherapistSessionNotesDataSnapshot:
        """
        ...
        try:
            logger.info(
                f"Starting therapist session notes data collection for {days} days"
            )
            ...
            logger.info(
                f"Therapist session notes data collection completed in {time.time() - start_time:.2f}s"
            )
            return snapshot
    ...
    def _collect_raw_session_data(self, user, days: int) -> Dict[str, Any]:
        ...
        try:
            ...
            session_notes = (
                SessionNote.objects.filter(session_filter)
                .select_related("therapist", "patient", "appointment")
    ...
    def _calculate_session_statistics(self, raw_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            frequency_stats = {
                "avg_sessions_per_patient": np.mean(
                    list(patient_session_counts.values())
                )
                if patient_session_counts
                else 0,
                "median_sessions_per_patient": np.median(
                    list(patient_session_counts.values())
                )
                if patient_session_counts
                else 0,
                "max_sessions_per_patient": max(patient_session_counts.values())
                if patient_session_counts
                else 0,
                "min_sessions_per_patient": min(patient_session_counts.values())
                if patient_session_counts
                else 0,
                "patients_with_multiple_sessions": sum(
                    1 for count in patient_session_counts.values() if count > 1
                ),
            }
            ...
            return {
                ...
                "completion_stats": {
                    ...
                    "cancelled_appointments": sum(
                        1 for apt in appointments if apt.status == "cancelled"
                    ),
                    "missed_appointments": sum(
                        1 for apt in appointments if apt.status == "missed"
                    ),
                },
            }
    ...
    def _analyze_notes_content(self, raw_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Quality metrics
            note_lengths = [
                len(note.notes) for note in session_notes if hasattr(note, "notes")
            ]
            quality_metrics = {
                ...
                "comprehensive_notes": sum(
                    1 for length in note_lengths if length > 500
                ),
            }
    ...
    def _analyze_therapeutic_content(self, session_notes: List) -> Dict[str, Any]:
        ...
        try:
            therapeutic_keywords = {
                "cbt": [
                    r"\bcbt\b",
                    r"cognitive.*behavior",
                    r"thought.*record",
                    r"behavioral.*activation",
                ],
                "dbt": [
                    r"\bdbt\b",
                    r"dialectical.*behavior",
                    r"mindfulness",
            ...
            return {
                ...
                "most_common_approach": max(
                    therapeutic_counts.items(), key=lambda x: x[1]
                )[0]
                if therapeutic_counts
                else None,
                "most_discussed_emotion": max(
                    emotion_counts.items(), key=lambda x: x[1]
                )[0]
                if emotion_counts
                else None,
                "overall_progress_trend": max(
                    progress_counts.items(), key=lambda x: x[1]
                )[0]
                if progress_counts
                else None,
            }
    ...
    def _analyze_progress_tracking(self, session_notes: List) -> Dict[str, Any]:
        ...
        try:
            ...
            for patient_id in patient_notes:
                patient_notes[patient_id].sort(
                    key=lambda x: x.session_date or x.timestamp
                )
            ...
            return {
                "avg_continuity_score": np.mean(continuity_scores)
                if continuity_scores
                else 0,
                "avg_goal_tracking_score": np.mean(goal_tracking_scores)
                if goal_tracking_scores
                else 0,
                "patients_with_multiple_sessions": len(continuity_scores),
                "excellent_tracking": sum(
                    1 for score in goal_tracking_scores if score > 0.7
                ),
                "poor_tracking": sum(
                    1 for score in goal_tracking_scores if score < 0.3
                ),
            }
    ...
    def _analyze_patient_care_patterns(
        ...
        try:
            ...
            therapeutic_approaches = self._analyze_patient_therapeutic_approaches(
                patient_data
            )
    ...
    def _analyze_engagement_patterns(self, patient_data: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            for patient_id, data in patient_data.items():
                ...
                avg_note_length = (
                    np.mean(
                        [len(note.notes) for note in notes if hasattr(note, "notes")]
                    )
                    if notes
                    else 0
                )
                ...
                engagement_score = (
                    ...
                    + (avg_note_length / 500)
                    * 0.3  # Note quality (normalized to 500 chars)
            ...
            return {
                ...
                "high_engagement_percentage": (high_engagement / max(1, total_patients))
                * 100,
                "medium_engagement_percentage": (
                    medium_engagement / max(1, total_patients)
                )
                * 100,
                "low_engagement_percentage": (low_engagement / max(1, total_patients))
                * 100,
    ...
    def _analyze_patient_therapeutic_approaches(
        ...
        try:
            ...
            return {
                ...
                "avg_approaches_per_patient": np.mean(patient_approach_diversity)
                if patient_approach_diversity
                else 0,
                "most_used_approach": max(approach_usage.items(), key=lambda x: x[1])[0]
                if approach_usage
                else None,
                "patients_with_multiple_approaches": sum(
                    1 for count in patient_approach_diversity if count > 1
                ),
                "approach_diversity_score": np.std(patient_approach_diversity)
                if patient_approach_diversity
                else 0,
            }
    ...
    def _analyze_session_outcomes(self, patient_data: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            for patient_id, data in patient_data.items():
                ...
                max_score_type = max(
                    patient_outcome_scores.items(), key=lambda x: x[1]
                )[0]
            ...
            return {
                ...
                "positive_outcome_percentage": (
                    positive_outcomes / max(1, total_patients)
                )
                * 100,
                "neutral_outcome_percentage": (
                    neutral_outcomes / max(1, total_patients)
                )
                * 100,
                "concerning_outcome_percentage": (
                    concerning_outcomes / max(1, total_patients)
                )
                * 100,
                "total_patients_analyzed": total_patients,
            }
    ...
    def _analyze_temporal_patterns(self, raw_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            patterns = self._analyze_session_timing_patterns(
    ...
    def _analyze_session_timing_patterns(
        self, session_notes: List, appointments: List
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "most_common_note_hour": max(set(note_hours), key=note_hours.count)
                if note_hours
                else None,
                "most_common_note_day": day_names[
                    max(set(note_days), key=note_days.count)
                ]
                if note_days
                else None,
                "most_common_appointment_hour": max(set(apt_hours), key=apt_hours.count)
                if apt_hours
                else None,
                "most_common_appointment_day": day_names[
                    max(set(apt_days), key=apt_days.count)
                ]
                if apt_days
                else None,
    ...
    def _analyze_workload_distribution(
        ...
        try:
            ...
            return {
                "avg_daily_notes": np.mean(daily_note_counts)
                if daily_note_counts
                else 0,
                "max_daily_notes": max(daily_note_counts) if daily_note_counts else 0,
                "avg_daily_appointments": np.mean(daily_apt_counts)
                if daily_apt_counts
                else 0,
                "max_daily_appointments": max(daily_apt_counts)
                if daily_apt_counts
                else 0,
                "workload_variability_notes": np.std(daily_note_counts)
                if daily_note_counts
                else 0,
                "workload_variability_appointments": np.std(daily_apt_counts)
                if daily_apt_counts
                else 0,
                "high_workload_days": sum(
                    1
                    for count in daily_note_counts
                    if count > np.mean(daily_note_counts) + np.std(daily_note_counts)
                )
                if daily_note_counts
                else 0,
                "total_working_days": len(
                    set(list(daily_notes.keys()) + list(daily_appointments.keys()))
                ),
            }
        ...
        try:
            ...
            for therapist_id, notes in therapist_data.items():
                if len(notes) > 1:
                    ...
                    dates = [
                        note.session_date or note.timestamp.date()
                        for note in notes
                        if hasattr(note, "session_date") or hasattr(note, "timestamp")
                    ]
                    if len(dates) > 1:
                        dates.sort()
                        intervals = [
                        ...
                        time_consistency = (
                            1 / (1 + np.std(intervals)) if intervals else 0
                        )
                    ...
                    if note_lengths:
                        quality_consistency = 1 / (
                            1 + np.std(note_lengths) / max(1, np.mean(note_lengths))
                        )
                        note_quality_consistency.append(quality_consistency)

            return {
                "avg_time_consistency": np.mean(therapist_consistency_scores)
                if therapist_consistency_scores
                else 0,
                "avg_quality_consistency": np.mean(note_quality_consistency)
                if note_quality_consistency
                else 0,
                "highly_consistent_therapists": sum(
                    1 for score in therapist_consistency_scores if score > 0.7
                ),
                "inconsistent_therapists": sum(
                    1 for score in therapist_consistency_scores if score < 0.3
                ),
        ...
        try:
            ...
            for note in session_notes:
                if (
                    hasattr(note, "notes")
                    and note.notes
                    and hasattr(note, "session_date")
                    and note.session_date
                    and hasattr(note, "patient")
                    and note.patient
                    and hasattr(note, "therapist")
                    and note.therapist
                ):
                    complete_notes += 1
