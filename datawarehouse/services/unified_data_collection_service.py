...
class UnifiedDataCollectionService:
    ...
    def _initialize_services(self):
        ...
        try:
            ...
            try:
                from .therapist_session_notes_service import (
                    TherapistSessionNotesCollectionService,
                )

                self.specialized_services["therapist"] = (
                    TherapistSessionNotesCollectionService()
                )
                logger.info("TherapistSessionNotesCollectionService initialized")
            except ImportError as e:
                logger.warning(
                    f"TherapistSessionNotesCollectionService not available: {e}"
                )
    ...
    def collect_unified_data(self, user_id: int, days: int = 30) -> UnifiedDataSnapshot:
        ...
        try:
            ...
            if "therapist" in self.specialized_services:
                try:
                    snapshot = self.specialized_services[
                        "therapist"
                    ].collect_therapist_session_data(user, days)
                    specialized_data["therapist_session_analytics"] = (
                        asdict(snapshot) if snapshot else {}
                    )
                except Exception as e:
            ...
            if "feeds" in self.specialized_services:
                try:
                    snapshot = self.specialized_services["feeds"].collect_feeds_data(
                        user, days
                    )
            ...
            if "mood_journal" in self.specialized_services:
                try:
                    snapshot = self.specialized_services[
                        "mood_journal"
                    ].collect_mood_journal_data(user, days)
            ...
            if "user_behavior" in self.specialized_services:
                try:
                    snapshot = self.specialized_services[
                        "user_behavior"
                    ].collect_user_behavior_data(user, days)
            ...
            unified_snapshot = UnifiedDataSnapshot(
                ...
                therapist_session_analytics=specialized_data.get(
                    "therapist_session_analytics"
                ),
            ...
            logger.info(
                f"Unified data collection completed for user {user_id} in {time.time() - start_time:.2f}s"
            )
    ...
    def collect_comprehensive_user_data(
        self, user_id: int, date_range: int = 30
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            ai_ready_data = {
                "mood_analytics": self._aggregate_mood_data(
                    user, date_range, unified_snapshot
                ),
                "journal_analytics": self._aggregate_journal_data(
                    user, date_range, unified_snapshot
                ),
                "communication_analytics": self._aggregate_communication_data(
                    user, date_range, unified_snapshot
                ),
                "therapy_session_analytics": self._aggregate_therapy_data(
                    user, date_range, unified_snapshot
                ),
                "behavioral_analytics": self._aggregate_behavioral_data(
                    user, date_range, unified_snapshot
                ),
                "social_analytics": self._aggregate_social_data(
                    user, date_range, unified_snapshot
                ),
                "processed_insights": self._generate_cross_domain_insights(
                    user, date_range, unified_snapshot
                ),
            }
            ...
            quality_metrics = self._calculate_data_quality(
                ai_ready_data, unified_snapshot
            )
            ...
            logger.info(
                f"Comprehensive data collection completed for user {user_id} in {time.time() - start_time:.2f}s"
            )
            return result
    ...
    def get_ai_ready_dataset(
        ...
        try:
            ...
            if use_cache:
                cached_dataset = (
                    AIAnalysisDataset.objects.filter(
                        user_id=user_id,
                        period_days=date_range,
                        is_active=True,
            ...
            comprehensive_data = self.collect_comprehensive_user_data(
                user_id, date_range
            )
    ...
    def _aggregate_mood_data(
        self, user, date_range: int, unified_snapshot
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            aggregated_data = {
                ...
                "mood_volatility": statistics.stdev(mood_scores)
                ...
                "quality_indicators": {
                    ...
                    "completeness": min(
                        1.0, len(mood_logs) / (date_range * 0.5)
                    ),  # Expect at least 0.5 entries per day for full score
                },
            }

            # Add specialized service data if available
            if unified_snapshot.mood_journal_analytics:
                aggregated_data["specialized_analytics"] = (
    ...
    def _aggregate_journal_data(
        ...
        try:
            ...
            aggregated_data = {
                ...
                "average_words_per_entry": statistics.mean(word_counts)
                if word_counts
                else 0,
                "median_words_per_entry": statistics.median(word_counts)
                if word_counts
                else 0,
                "writing_consistency": self._calculate_writing_consistency(
                    journal_entries
                ),
                ...
                "quality_indicators": {
                    ...
                    "temporal_coverage": len(
                        set(e.timestamp.date() for e in journal_entries)
                    )
                    / date_range,
                    "completeness": min(
                        1.0, len(journal_entries) / (date_range * 0.3)
                    ),  # Expect at least 0.3 entries per day
                },
            }
    ...
    def _aggregate_communication_data(
        self, user, date_range: int, unified_snapshot
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            aggregated_data = {
                ...
                "date_range": date_range,
                "specialized_analytics": unified_snapshot.feeds_analytics
    ...
    def _aggregate_therapy_data(
        ...
        try:
            aggregated_data = {
                ...
                "date_range": date_range,
                "specialized_analytics": unified_snapshot.therapist_session_analytics
    ...
    def _aggregate_behavioral_data(
        self, user, date_range: int, unified_snapshot
    ) -> Dict[str, Any]:
        ...
        try:
            aggregated_data = {
                "status": "from_specialized_service",
                "date_range": date_range,
                "specialized_analytics": unified_snapshot.user_behavior_analytics
    ...
    def _calculate_data_quality(
        self, ai_ready_data: Dict[str, Any], unified_snapshot
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for domain in [
                ...
                readiness_flags[
                    f"ready_for_{domain.replace('_analytics', '_analysis')}"
                ] = quality_scores[domain] >= 0.3
            ...
            overall_completeness = min(
                1.0, overall_quality * 1.2
            )  # Slight boost for having multiple sources
            ...
            return {
                ...
                "analysis_recommendation": "suitable"
    ...
    def _cache_ai_dataset(
        self, user_id: int, period_days: int, comprehensive_data: Dict[str, Any]
    ):
        ...
        try:
            ...
            dataset = AIAnalysisDataset.objects.create(
                ...
                communication_metrics=comprehensive_data.get(
                    "communication_analytics", {}
                ),
                therapy_session_data=comprehensive_data.get(
                    "therapy_session_analytics", {}
                ),
                ...
                processing_duration_seconds=processing_metadata.get(
                    "collection_time_seconds", 0
                ),
                ready_for_mood_analysis=quality_metrics.get("readiness_flags", {}).get(
                    "ready_for_mood_analysis", False
                ),
                ready_for_journal_analysis=quality_metrics.get(
                    "readiness_flags", {}
                ).get("ready_for_journal_analysis", False),
                ready_for_behavior_analysis=quality_metrics.get(
                    "readiness_flags", {}
                ).get("ready_for_behavioral_analysis", False),
                ready_for_communication_analysis=quality_metrics.get(
                    "readiness_flags", {}
                ).get("ready_for_communication_analysis", False),
                ready_for_therapy_analysis=quality_metrics.get(
                    "readiness_flags", {}
                ).get("ready_for_therapy_session_analysis", False),
                expires_at=expires_at,
            )

            logger.info(
                f"Cached AI dataset for user {user_id}, expires at {expires_at}"
            )
            return dataset
    ...
    def _create_empty_comprehensive_dataset(
        self, user_id: int, date_range: int, error: str = None
    ) -> Dict[str, Any]:
    ...
    def _calculate_mood_trend(self, daily_moods: Dict) -> Dict[str, Any]:
        ...
        numerator = sum(
            (i - x_mean) * (y - y_mean) for i, y in enumerate(daily_averages)
        )
