...
class FeatureUsageMetricsViewSet(viewsets.ModelViewSet):
    ...
    def engagement_stats(self, request):
        ...
        total_time = queryset.aggregate(total=Sum('total_time_spent_minutes'))['total'] or 0
...
class UnifiedDataCollectionViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        try:
            ...
            summary_data = {
                ...
                'total_mood_entries': queryset.aggregate(total=Sum('mood_entries_count'))['total'] or 0,
                'total_journal_entries': queryset.aggregate(total=Sum('journal_entries_count'))['total'] or 0,
            }
    ...
    def create(self, request):
        ...
        try:
            ...
            unified_snapshot = unified_collector.collect_unified_data(user_id, days, force_refresh)
            ...
            return Response({'error': f'Failed to create collection: {str(e)}'}, 
                          status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def retrieve(self, request, pk=None):
        ...
        try:
            ...
            return Response({'error': f'Unified data collection with id {pk} not found'}, 
                          status=status.HTTP_404_NOT_FOUND)
        except Exception as e:
            logger.error(f"Error retrieving unified data collection {pk}: {str(e)}")
            return Response({'error': f'Failed to retrieve collection: {str(e)}'}, 
                          status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def list(self, request):
        ...
        try:
            ...
            if health_status['summary']['unhealthy_services'] > 0:
                health_status['status'] = 'degraded' if health_status['summary']['healthy_services'] > 0 else 'unhealthy'
...
class DatawarehouseDashboardView(viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        try:
            ...
            return Response({'error': f'Failed to generate dashboard: {str(e)}'}, 
                          status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def _get_engagement_stats(self, start_date, end_date):
        ...
        total_mood_entries = snapshots.aggregate(total=Sum('mood_entries_count'))['total'] or 0
        total_journal_entries = snapshots.aggregate(total=Sum('journal_entries_count'))['total'] or 0
        total_messages = snapshots.aggregate(total=Sum('messages_sent'))['total'] or 0
        
        avg_engagement = snapshots.aggregate(avg=Avg('social_engagement_score'))['avg'] or 0
    ...
    def _get_trending_stats(self, start_date, end_date):
        ...
        improving_users = recent_snapshots.filter(mood_trend='improving').values('user').distinct().count()
        declining_users = recent_snapshots.filter(mood_trend='declining').values('user').distinct().count()
...
class DataExportView(viewsets.ViewSet):
    ...
    def system_summary(self, request):
        ...
        try:
            ...
            return Response({'error': f'Summary export failed: {str(e)}'}, 
                          status=status.HTTP_500_INTERNAL_SERVER_ERROR)
