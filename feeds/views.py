...
from feeds.models import (
    Post, Comment, Topic, Reaction, PollOption, PollVote
)
...
class PostViewSet(viewsets.ModelViewSet):
    ...
    def _apply_filters(self, queryset):
        ...
        if min_reactions and min_reactions.isdigit():
            ...
            ).filter(total_reactions__gte=int(min_reactions))
    ...
    @extend_schema(
        ...
        responses={
            200: OpenApiResponse(description="List of reactions")
        },
        request=ReactionActionSerializer,
        methods=['POST']
    )
    @action(detail=True, methods=['get', 'post'], serializer_class=ReactionActionSerializer)
    def react(self, request, pk=None):
        ...
        return Response({
            'detail': 'Reaction added successfully' if created else 'Reaction updated successfully',
            'reaction': {
