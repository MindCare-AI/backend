...
class ConversationAdmin(admin.ModelAdmin):
    list_display = ('id', 'conversation_type', 'group_name_display', 'participants_count', 
                   'messages_count', 'created_at', 'is_active')
...
class MessageAdmin(admin.ModelAdmin):
    list_display = ('id', 'formatted_content', 'sender_link', 'conversation_link', 
                   'message_type', 'timestamp', 'is_chatbot', 'read_count')
...
class ReactionAdmin(admin.ModelAdmin):
    ...
    def message_preview(self, obj):
        content = obj.message.content[:30] + '...' if len(obj.message.content) > 30 else obj.message.content
        url = reverse('admin:messaging_message_change', args=[obj.message.id])
        return format_html('<a href="{}" title="{}">{}</a>', url, obj.message.content, content)
