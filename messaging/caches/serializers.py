...
class MessageSerializer:
    ...
    def _serialize_message(cls, message) -> Dict:
        ...
        try:
            ...
            logger.error(f"Error serializing message {getattr(message, 'id', 'unknown')}: {str(e)}", exc_info=True)
            # Return minimal valid data
            return {
                'id': getattr(message, 'id', 0),
                'content': getattr(message, 'content', ''),
                'conversation_id': getattr(message, 'conversation_id', 0),
                'timestamp': cls._format_datetime(getattr(message, 'timestamp', timezone.now())),
                'error': str(e)
            }
    ...
    def _serialize_conversation(cls, conversation) -> Dict:
        ...
        try:
            ...
            if hasattr(conversation, 'participants') and conversation.participants.exists():
                participants_data = []
                ...
                data['participant_ids'] = list(conversation.participants.values_list('id', flat=True))
            ...
            logger.error(f"Error serializing conversation {getattr(conversation, 'id', 'unknown')}: {str(e)}", exc_info=True)
            # Return minimal valid data
            return {
                'id': getattr(conversation, 'id', 0),
                'type': conversation.__class__.__name__,
                'created_at': cls._format_datetime(getattr(conversation, 'created_at', timezone.now())),
                'error': str(e)
            }
