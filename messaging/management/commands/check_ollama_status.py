    ...
    def handle(self, *args, **kwargs):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.stdout.write(
                    self.style.ERROR("✗ Ollama server returned unexpected response")
                )
                return
        ...
        try:
            ...
            if models:
                self.stdout.write(
                    self.style.SUCCESS(
                        f"✓ Available models: {', '.join([m['name'] for m in models])}"
                    )
                )
            else:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                self.stdout.write(
                    self.style.WARNING("✗ NVIDIA GPU not detected or accessible")
                )
        except FileNotFoundError:
            self.stdout.write(
                self.style.WARNING("✗ nvidia-smi not found, GPU may not be available")
            )
