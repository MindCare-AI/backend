    ...
    def handle(self, *args, **kwargs):
        ...
        try:
            ...
            if self._check_model_exists(model_name):
                self.stdout.write(self.style.SUCCESS(f'Model "{model_name}" is already downloaded.'))
                return
            ...
            self.stderr.write(self.style.ERROR('Ollama is not installed or not found in PATH.'))
        except subprocess.CalledProcessError as e:
            self.stderr.write(self.style.ERROR(f'Command failed: {e.stderr if hasattr(e, "stderr") else str(e)}'))
