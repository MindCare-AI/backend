...
from .tasks import generate_chatbot_response
        ...
        if conversation.conversation_type == 'chatbot':
            try:
                ...
                history = list(Message.objects.filter(
                    conversation=conversation
                ).order_by('-timestamp')[:3].values('content', 'is_chatbot'))
                
                # Format history in the way get_ollama_response expects
                formatted_history = [
                    {"content": msg["content"], "response": "User response" if msg["is_chatbot"] else "Bot response"} 
                    for msg in history
                ]
...
class GroupManagementView(APIView):
    ...
    def post(self, request, group_id):
        ...
        conversation = get_object_or_404(
            Conversation.objects.filter(conversation_type='group'),
            id=group_id
        )
