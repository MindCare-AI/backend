...
class ChatbotConversationViewSet(viewsets.ModelViewSet):
    ...
    def send_message(self, request, pk=None):
        ...
        user_message = serializer.save(sender=request.user, conversation=conversation)

        # Convert last 3 bot messages to a list of dicts (history)
        history_qs = conversation.messages.filter(is_bot=True).order_by("-timestamp")[
            :3
        ]
        history = list(history_qs.values("sender", "content"))

        bot_response = get_chatbot_response(user_message.content, history)
