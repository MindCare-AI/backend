...
try:
    ...
    # Django-dependent imports (must be after django.setup())
...
def test_ai_datawarehouse_integration():
    ...
    try:
        ...
        if users:
            ...
            print(
                f"   📈 Quality score: {dataset.get('quality_metrics', {}).get('overall_quality', 0.0):.2f}"
            )
            print(
                f"   📋 Processing version: {dataset.get('processing_metadata', {}).get('processing_version', 'unknown')}"
            )
        else:
    ...
    try:
        ...
        print("   ✅ Tips service initialized")
    ...
    try:
        ...
        print("   ✅ Social analysis service initialized")
