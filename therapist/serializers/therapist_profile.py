    ...
    def to_internal_value(self, data):
        ...
        for day, slots in data.items():
            ...
            if not isinstance(slots, list):
                raise serializers.ValidationError(f"Schedule for {day_lower} must be a list of time slots.")

            validated_slots = []
            for slot in slots:
                if not isinstance(slot, dict) or "start" not in slot or "end" not in slot:
                    raise serializers.ValidationError(f"Invalid time slot format in {day_lower}. Expected {{'start': 'HH:MM', 'end': 'HH:MM'}}.")

                try:
                    start_time = datetime.strptime(slot["start"], "%H:%M").time()
                    end_time = datetime.strptime(slot["end"], "%H:%M").time()
                    if start_time >= end_time:
                        raise serializers.ValidationError(f"Start time must be before end time in {day_lower} for slot {slot}.")
                    validated_slots.append({"start": slot["start"], "end": slot["end"]})
                except ValueError:
                    raise serializers.ValidationError(f"Invalid time format in {day_lower}. Use HH:MM format.")
...
class TherapistProfileSerializer(serializers.ModelSerializer):
    ...
    years_of_experience = serializers.IntegerField(min_value=0, max_value=100, required=False)
...
class TherapistVerificationSerializer(serializers.Serializer):
    ...
    def validate_license_image(self, value):
        ...
        if mime_type not in settings.VERIFICATION_SETTINGS['IMAGE_REQUIREMENTS']['ALLOWED_MIME_TYPES']:
