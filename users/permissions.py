...
class IsSuperUserOrSelf(permissions.BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        ...
        logger.debug(f"Permission check: user={request.user.username}, is_self={is_self}")
        return is_self
...
class IsTherapistForPatient(permissions.BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        ...
        if hasattr(obj, 'user') and obj.user.user_type == 'patient':
            ...
            logger.debug(f"Therapist permission check: therapist={request.user.username}, patient={obj.user.username}, access_granted={is_therapist}")
            return is_therapist
